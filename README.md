...
> **하위 호환이 깨진 지점 2가지** — 이전 버전은 `--review-budget`을 경고만 내고
> 무시했으므로 **무엇을 주든 종료 코드 0으로 번역이 나왔습니다.** 지금은 값을 검증하고
> 실행을 중단시킬 수 있습니다: ① `--review-budget abc`처럼 읽을 수 없는 값과
> ② `--to th --review-budget 10%`처럼 프로파일 있는 대상 언어가 하나도 없는 호출이
> **종료 코드 2로 끝나고 번역 산출물이 나오지 않습니다.** 둘 다 의도된 동작이지만,
> 옛 문서를 읽고 스크립트를 짰다면 이 두 줄을 확인하세요.
