...
**실제 검수 비율 10%에서 무작위 대비 약 7.4배**(en-ko **7.32x** · ja-ko **7.56x**)의
오류 포착률을 냅니다. 공개 병렬 코퍼스 TED2020(OPUS)에서 표본 5,000건씩을 뽑아
합성 오류를 주입하고 측정한 결과입니다(2026-09-04).
...
| --- | --- | --- | --- | --- | --- | --- |
| 1% | 6.32% | 54.60% | 8.64x | 5.66% | 49.40% | 8.73x |
| 2% | 6.32% | 54.60% | 8.64x | 5.66% | 49.40% | 8.73x |
| 5% | 6.32% | 54.60% | 8.64x | 5.66% | 49.40% | 8.73x |
| **10%** | **10.00%** | **73.20%** | **7.32x** | **10.00%** | **75.60%** | **7.56x** |
| 20% | 20.00% | 86.40% | 4.32x | 20.00% | 83.80% | 4.19x |
| 30% | 30.00% | 88.00% | 2.93x | 30.00% | 84.80% | 2.83x |

예산 1~5% 행이 모두 같은 값인 것은 버그가 아닙니다 — hard fail이 예산을 우회해
무조건 큐에 들어가므로, 요청 예산이 hard fail만으로 채워지는 실제 검수 비율(en-ko
6.32% · ja-ko 5.66%)보다 낮은 동안은 요청을 더 낮춰도 결과가 바뀌지 않습니다.
...
직전 측정([en-ko](bench/results/en-ko-2026-07-29.md) ·
[ja-ko](bench/results/ja-ko-2026-07-29.md))도 남겨 두었으나 **정답지가 달라
직접 비교할 수 없습니다** — `negation` 주입기를 삽입 전용에서 제거 전용으로
고치면서 7개 유형의 세그먼트 배정이 함께 바뀌었습니다(리포트의 "측정 방법의 한계" 절).
...
**무작위보다도 못합니다**(1.41% vs 무작위 10.28%). 다른 오류를 상위로 올리면서
문법적으로 완벽한 문장을 오히려 큐에서 밀어내기 때문입니다 — Tier 1 투자가
필요한 이유입니다(요구사항정의서 §12 Q4). **이 숫자는 융합 방식을 noisy-or로
바꾸고 숫자 검출을 고친 뒤에도, 나아가 `negation` 정답지를 통째로 다시 만든
뒤에도 1.41% 그대로였습니다** — 결정론적 신호를 어떻게 조합하든 의미 판단에는
닿지 못한다는 뜻입니다. 정답지 수정으로 오히려 **예산 30%의 격차가 드러났습니다**
(29.58% vs 29.68%로 구분 불가였던 것이 19.72% vs 30.44%가 됐습니다) — 옛 정답지가
일본어 문장에 영어 `not`을 끼워 넣던 탓에 Tier 0 신호가 그 이물질을 우연히
잡아 Recall이 부풀어 있었습니다. 또한 **ko 자막의 약 절반(en-ko
...
## CLI (`check`·`translate`·`transcribe` 구현 완료)
...
| `1` | **규격 위반 발견** | `check`만 |
| `3` | **번역되지 않은 세그먼트가 남음 — 원문 유지** | `translate`만 |
...
| `69` | **외부 서비스(LLM·STT 프로바이더)가 요청을 거부함** - 인증 실패·모델 없음 등 | `translate`·`transcribe` |
| `70` | **산출물의 내용 결함** - 자막·검수 리포트를 쓰려는데 그 **내용**이 파일로 나갈 수 없는 값을 담고 있음(모델이 낸 고립 서로게이트, JSON으로 바꿀 수 없는 값 등) | 공통 |
...
**`3`은 `1`에서 갈라져 나왔습니다.** 예전에는 `check`의 위반과 `translate`의 번역 실패가
같은 `1`이라 CI가 두 사건에 같은 대응을 했습니다. `69`와 가르는 축은 **"산출물이
나갔는가"** 입니다 — `69`는 그 언어가 통째로 죽은 것이고 `3`은 파일이 나왔는데 일부 줄이
원문으로 남은 것입니다.

**`3`만 `sysexits.h` 밖의 값입니다.** 거기에는 "산출물은 나왔는데 일부가 비었다"에 해당하는
값이 없습니다. 뜻이 가장 가까운 `75`(`EX_TEMPFAIL`, "다시 시도하라")를 쓰지 않은 것은
**사유 3종의 처방이 서로 다르기 때문입니다** — `provider_error`는 재실행,
`invalid_response`는 모델 교체, `empty_translation`은 원문 확인입니다. 코드 하나가 그중
"재시도"만 주장하면 나머지 둘에서 거짓이 됩니다. 실제로 `empty_translation`은 계약을 지킨
응답이라 캐시가 보존하고, 같은 설정의 재실행은 캐시 히트로 같은 결과를 냅니다.
이 코드는 "번역되지 않은 세그먼트가 남았다"만 말하고 **재시도 여부는 주장하지 않습니다** —
그 판단의 근거인 사유(`provider_error`·`invalid_response`·`empty_translation`)는
화면의 실패 세그먼트 줄에 나갑니다.
...
봐야 할 자막"의 목록입니다. 셋 중 **하나를 주면** 번역 직후에 Tier 0 신호 9종이 돌고
...
cuesift translate ep01.ko.srt --to en    --review-top-k 50       # 상위 50개를 큐에
```

| 옵션 | 받는 값 | 비고 |
|---|---|---|
| `--review-budget` | **비율만** — `10%` · `0.1` · `0` · `100%` | `50`처럼 **개수 지정은 종료 코드 2**입니다. 개수를 원하면 아래 `--review-top-k`를 씁니다 - `k/n`으로 환산하면 올림과 hard fail 소진 때문에 정확히 K개가 나오지 않아 옵션이 거짓말을 하게 되므로 환산하지 않습니다 |
| `--review-threshold` | `0.0`~`1.0` | 이 위험도 이상을 큐에 담습니다 |
| `--review-top-k` | **0 이상의 정수** - `50` · `0` | 위험도 상위 K개를 큐에 담습니다. 음수는 종료 코드 2이고 **상한은 없습니다** - 세그먼트 수보다 큰 값은 전량 선별이지 오류가 아닙니다 |
| `--review-out` | 디렉터리 | 결과를 파일로 남깁니다(아래). **위 셋 중 하나와 함께** 써야 하고, 단독으로 주면 종료 코드 2입니다 |
| `--review-format` | `json`(기본) · `html` · `both` | 리포트 형식입니다. `html`은 브라우저에서 여는 `report.html`, `both`는 둘 다. **`--review-out`과 함께** 써야 하고, 단독으로 주면 종료 코드 2입니다 |

위 표는 **큐에 무엇을 담을지 고르는 옵션 셋과 그 결과를 어디에 어떤 형식으로 낼지
정하는 옵션 둘**로 나뉩니다.

**둘 이상을 함께 주면 종료 코드 2입니다.** FR-6.3은 "두 방식으로 지정할 수 있다"이지
"동시에"가 아니고, 합성하면 어느 쪽이 이겼는지가 출력에서 사라집니다.

**`--review-top-k`가 준 K보다 많이 나올 수 있습니다.** hard fail은 예산을 우회하므로
(FR-6.2) hard fail이 K개를 넘으면 그만큼 담깁니다. 비율 축도 같은 성질이고, 그래서
요약이 **실제 검수 개수와 실제 검수 비율을 함께** 냅니다 - 그 숫자를 보면 됩니다.

**`--review-top-k`는 `--tier1`과 함께 쓸 수 없습니다** (종료 코드 2). Tier 1 경로가
비율 예산을 필수로 요구하기 때문이고, 그때는 `--review-budget`을 씁니다.
...
9종으로 예산 10%에서 **1.41%**만 잡히고, 이것은 같은 예산의 무작위(10.28%)보다도 낮습니다
...
Tier 1까지 가지 못합니다(실측: 죽은 엔드포인트로 `--tier1`을 돌리면 exit 3). 69는
...
#### Tier 1 역번역 — 아직 파이프라인에 붙지 않았습니다 (FR-4.2, 🟡)

역번역 신호 자체는 의미 반전을 정확히 가려냅니다(negation 세그먼트의 역번역 코사인 0.69 대 0.78, 방향 올바름). 하지만 Tier 1 후보 선정이 Tier 0 위험도 순위에 의존해 의미 반전을 무작위 위치에 놓아 신호가 볼 기회를 얻지 못합니다(실측: 예산 10% negation Recall 1.41% → 0.00%). 다음 작업이 이 후보 선정을 재설계합니다. 상세: [`bench/results/en-ko-2026-09-05.md`](bench/results/en-ko-2026-09-05.md).

#### 검수 리포트 파일 — `--review-out` · `--review-format` (FR-7.2·FR-7.3, 동작합니다)

**화면 요약은 실행이 끝나면 사라집니다.** `--review-out DIR`을 주면 같은 결과가
파일로 남습니다 — 검수자에게 넘기고, 스크립트로 읽고, 며칠 뒤에 다시 엽니다.
**형식은 둘입니다**: 기계가 읽는 `review.json`(기본)과 사람이 브라우저에서 여는
`report.html`(`--review-format html`). 아래는 먼저 JSON이고, HTML은 이 절 끝에
따로 있습니다.
...
| `--review-format html` + `--review-out` + 예산 | `{stem}.{언어}.report.html` **하나만** — `.review.json`은 나오지 않습니다 |
| `--review-format both` + `--review-out` + 예산 | 파일 **둘** — 같은 stem으로 `.review.json`과 `.report.html`이 나란히 |
| `--review-format` **단독**(`--review-out` 없이) | **종료 코드 2** — 낼 곳이 없는 형식 지정입니다. 조용히 무시하면 나오지 않는 파일을 찾아 헤매고, 종료 코드가 0이라 스크립트는 성공으로 읽습니다 |
| `--review-format html` + 예산 + `--dry-run` | 종료 코드 0, 파일은 안 나오고 **예고 줄이 `.report.html`을 가리킵니다** — 형식과 무관하게 `.review.json`을 예고하던 판이 있었습니다(리뷰에서 실측으로 잡혔습니다) |

##### `report.html` — 브라우저에서 여는 검수 리포트 (FR-7.3)

```bash
cuesift translate ep01.ko.srt --to en --review-budget 30% \
    --review-out reports --review-format both
```

```text
  리포트 reports\ep01.en.review.json
  리포트 reports\ep01.en.report.html
```

**의존성이 0개인 단일 파일입니다.** CSS와 JS가 파일 안에 들어 있어 검수자에게
그 파일 하나만 넘기면 됩니다 — 서버도, 네트워크도, 빌드도 필요 없습니다.

| 무엇 | 내용 |
|---|---|
| **요약** | 총 세그먼트 수 · 검수 대상 수와 **실제 비율** · hard fail 수 · 신호별 적발 건수. 화면 요약·`review.json`과 **같은 객체**에서 나오므로 수치가 갈라질 자리가 없습니다 |
| **원문/번역 대조** | 세그먼트마다 타임코드 · 원문 · 번역문을 한 행에 나란히 |
| **위험 구간 하이라이트** | 신호가 지목한 구간만 `<mark>`로 칠합니다. `spans[].side`가 원문 칸인지 번역 칸인지를 가릅니다 |
| **필터** | 신호 이름 체크박스와 hard fail 토글로 행을 줄이고, `표시 중 N / M` 카운터가 함께 움직입니다 |

**하이라이트가 붙는 신호는 3종뿐입니다** — `glossary.miss` · `struct.number_missing` ·
`struct.tag_lost`. 나머지 7종은 **판정 단위가 세그먼트 전체**라 구간 개념이 성립하지
않고, 그 행은 신호 배지로만 표시됩니다. 빈 것이 아니라 없는 것입니다.

**파일명은 `{stem}.{대상언어}.report.html`로 `review.json`과 같은 규칙입니다.**
갈라지면 `both`가 낸 두 파일이 한 실행의 산출물이라는 것을 눈으로 맞출 수 없습니다.

**JS를 끈 브라우저에서도 열립니다.** 필터가 동작하지 않는다는 안내가 뜨고 **전량이
표시됩니다** — 행을 숨기는 `hidden` 속성을 JS만 붙이므로, 스크립트가 없으면 붙일 주체가
없습니다. 하이라이트는 CSS라 그대로 남습니다.

> **필터 동작에는 자동 테스트가 없습니다.** pytest는 브라우저를 띄우지 않고,
> 헤드리스 브라우저를 넣으면 이 프로젝트의 dev 의존성 고정(3개)이 깨집니다.
> 파이썬이 보장하는 것은 **마크업 계약까지**이고, 필터가 실제로 행을 줄이는 것과
> `noscript` 폴백은 Chrome에서 **1회 눈으로** 확인했습니다. 검사하지 않는 것을
> 검사한다고 적지 않습니다.
...
**종료 코드는 `3`입니다** — 전량 실패도 "번역되지 않은 세그먼트가 남았다"이지 리포트의
실패가 아닙니다. 리포트는 정상적으로 나왔습니다.
...
#### 진행 표시 — `--progress` · `CUESIFT_PROGRESS` (FR-8.5, 동작합니다)

**기본으로 켜져 있고, 별도로 지정하지 않으면 환경을 감지해 형태만 바꿉니다.**
번역은 세그먼트가 수천 개면 수십 분이 걸리는데, 그동안 화면이 조용하면 "느린 것"과
"멈춘 것"을 구별할 방법이 없습니다.

**출력은 전부 stderr로 나갑니다.** stdout은 파이프로 받는 쪽의 것이라
`cuesift translate ... > result.txt`로 받아도 진행 표시가 섞이지 않습니다.
설계 판단의 근거는 [진행 표시 설계 스펙](docs/superpowers/specs/2026-08-29-progress-display-design.md)에 있습니다.

##### 켜고 끄기 — 세 곳에서 정할 수 있습니다

```mermaid
flowchart TD
    A["--progress / --no-progress"] -->|지정됨| Z{{"켬 / 끔"}}
    A -->|없음| B["CUESIFT_PROGRESS"]
    B -->|지정됨| Z
    B -->|없음| C["cuesift.yaml의 output.progress"]
    C -->|지정됨| Z
    C -->|없음| D["기본값 — 켬"]
    D --> Z
    Z -->|끔| OFF["출력 없음"]
    Z -->|켬| S{"stderr이 대화형인가"}
    S -->|예| I["interactive — 한 줄 갱신"]
    S -->|아니오| P["plain — 이정표 누적"]
```

우선순위는 다른 옵션과 같은 **CLI 인자 > 환경변수 > `cuesift.yaml` > 기본값**입니다.

| 지정 수단 | 켜는 값 | 끄는 값 |
| --- | --- | --- |
| CLI 플래그 | `--progress` | `--no-progress` |
| 환경변수 | `CUESIFT_PROGRESS=1` (아래 거짓값이 아닌 **모든** 문자열) | `CUESIFT_PROGRESS=0` · `false` · `no` · `off` · 빈 문자열 |
| 설정 파일 | `output: {progress: true}` | `output: {progress: false}` |

**환경변수의 거짓값은 대소문자를 가리지 않고 앞뒤 공백도 무시합니다.** 그 다섯 가지가
아니면 전부 참이므로 `CUESIFT_PROGRESS=off`는 끄지만 `CUESIFT_PROGRESS=disabled`는
**켭니다** — 판독 규칙이 한 함수(`progress.env_flag`)에만 있어 이 목록이 곧 전부입니다.

##### 플래그는 켜고 끄기만 정하고, 형태는 언제나 감지가 정합니다

`--progress`를 CI에서 명시해도 캐리지 리턴 갱신이 나오지 않습니다. 셋 중 **하나라도**
해당하면 비대화형으로 보고 `plain`을 씁니다.

| 신호 | 왜 이것을 보나 |
| --- | --- |
| `stderr`이 TTY가 아니다 | 파이프·리다이렉션입니다. 표준 신호입니다 |
| 환경변수 `CI`가 비어 있지 않다 | GitHub Actions·GitLab·CircleCI가 공통으로 세웁니다. **TTY를 할당하는 러너**(`docker run -t`, 일부 self-hosted)에서 `\r`이 로그 파일에 그대로 남는 것을 막습니다 |
| `TERM=dumb` | Emacs 셸과 일부 편집기 터미널이 `\r` 갱신을 렌더하지 못합니다 |

**`NO_COLOR`는 보지 않습니다.** 색에 관한 규격인데 이 렌더러는 색을 쓰지 않으므로,
신호로 삼으면 규격을 넘어 해석하는 것이 됩니다.

##### 실제 화면

**plain** — 제어문자를 전혀 쓰지 않고 이정표를 누적합니다. 아래는 400세그먼트 자막의
실측입니다.
...
**이정표는 10%p마다이고 100%는 언제나 냅니다.** 간격만 두면 마지막 조각이 10%p에 못
미칠 때 진행이 97%에서 끝난 것처럼 보입니다. 반대로 간격을 좁히면 세그먼트 4000개짜리
작업이 언어당 수백 줄을 뱉습니다.

**진척은 배치가 끝날 때마다 갱신되므로 자막이 짧으면 간격이 10%p보다 벌어집니다.**
배치 크기가 10이라 26세그먼트짜리는 이렇게 나옵니다 — 10%p 규칙이 잴 수 있는 것보다
한 걸음이 더 큽니다. 대상 언어가 여럿이면 언어마다 처음부터 반복하고, `--review-out`을
주면 리포트 단계가 뒤에 붙습니다.
...
**interactive** — 같은 줄을 `\r`로 덮어쓰고, 단계가 끝나면 개행해 확정합니다.
점선은 단계 이름 뒤의 열을 맞추는 장식입니다.

```text
[en] 번역 ............... 26/26 (100%)     ← 이 줄이 제자리에서 갱신됩니다
[en] 번역 ............... 완료 (실패 0)     ← 확정되고 다음 줄로 넘어갑니다
```

##### 단계는 셋이고 대상 언어마다 반복됩니다

| 라벨 | 언제 나오나 | 진척의 단위 |
| --- | --- | --- |
| `[언어] 번역` | 언제나 | 번역이 끝난 세그먼트 수 / 전체 |
| `[언어] Tier 1` | `--tier1`을 켰을 때 | 자가일관성을 잰 세그먼트 수 / 후보 수 |
| `[언어] 리포트` | `--review-out`을 줬을 때 | 진척 없이 `기록 완료`만 |

**`Tier 1 완료`만 있고 진척 줄이 없는 실행은 정상입니다.** 회색지대 후보가 0건이면
잴 대상이 없어 단계는 열리고 닫히기만 합니다 — 스위치가 안 먹은 것이 아닙니다.
같은 구분이 `review.json`의 `cost.includes`에도 나타나며, 그쪽은 후보 0건일 때
`"tier1"`을 **싣지 않습니다**.

**진행 줄과 경고는 겹치지 않습니다.** 용어집 실패나 캐시 경고가 번역 도중에 나가면
떠 있던 `\r` 줄을 먼저 지우고 씁니다.

**닫힌 파이프에서도 종료 코드가 흐려지지 않습니다.** `cuesift translate ... 2>&1 | head -1`
처럼 stderr를 잘라 읽어도 진행 표시가 예외를 올리지 않고 스스로 비활성화됩니다 —
진행 줄은 부수적이고, 실제 결과와 종료 코드가 그 때문에 바뀌면 안 됩니다.

### 설정 파일 `cuesift.yaml` — 모든 옵션 (FR-8.4, 동작합니다)

**`--config`는 `translate`의 옵션이 아니라 `cuesift` 바로 뒤에 붙는 최상위 옵션입니다**
(`cuesift --config c.yaml translate ...` — 서브커맨드 뒤에 쓰면 `No such option`으로
종료 코드 2가 납니다). 경로를 주지 않아도 **현재 디렉터리의 `./cuesift.yaml`을 자동으로
읽습니다.** 설정이 적용되면 출처 한 줄이 stderr에 나갑니다.

```bash
cuesift translate a.ko.srt --dry-run          # ./cuesift.yaml 자동 적용
cuesift --config ./ci.yaml check a.ja.srt     # 명시 지정
```

전체 키 목록은 [요구사항정의서 §8.2](docs/요구사항정의서.md)에 있습니다. 규칙은 넷입니다.

| 항목 | 규칙 |
| --- | --- |
| 우선순위 | CLI 인자 > 환경변수 > `cuesift.yaml` > 기본값 |
| 자동 탐색 | 현재 디렉터리 한 칸만. **상위 디렉터리로 올라가지 않습니다** |
| 모르는 키 | 종료 코드 2로 거부하고 가까운 키를 제시합니다 |
| 부분 지정 | 명시하지 않은 키는 기본값을 유지합니다 |

**상위 디렉터리를 읽지 않는 것은 의도입니다.** 존재를 모르는 파일이 검수 기준
(hard fail 임계·신호 가중치)을 바꾸면 Recall@Budget 수치가 조용히 오염됩니다.

**모르는 키를 조용히 무시하지 않는 것도 의도입니다.** `review_budgt` 오타 하나가
무시되면 트리아지가 아예 돌지 않았는데 종료 코드는 `0`이 나갑니다 — 이 저장소가
1급으로 금지하는 "검사하지 않고 통과하는 게이트"입니다.

**신호 가중치는 설정 파일 전용입니다.** `signals.weights`에 실제 신호 이름
(`spec.violation`·`struct.tag_lost` 등 10종, `review.json`의 `signals[].name`과 같습니다)
...
### `cuesift transcribe` - 영상에서 원문 자막 만들기 (동작합니다)
...
옵션은 넷입니다. `transcribe`와 `translate`가 **같은 이름**을 씁니다.

| 옵션 | 뜻 | 기본값 |
| --- | --- | --- |
| `--source-lang` | 원문 언어 | `ko` |
| `--out` | 출력 디렉터리 | 입력 파일과 같은 곳 |
...
하나를 반드시 못 쓰게 됩니다. `cuesift.yaml`에서는 `stt.base_url`·`stt.model`이
그 자리이고, `translate`에 영상을 주는 것은 `input.media`입니다.

**429·5xx는 자동으로 다시 부릅니다.** 어댑터가 `Retry-After`를 파싱해 실어 주고
`stt/retry.py`가 그것을 받아 백오프합니다 - 재시도 대기 줄이 stderr에 나옵니다.
소진하면 종료 코드 `69`입니다.

`transcribe`와 `translate --media`는 **같은 함수 하나를** 부릅니다. 출력 경로
규칙과 재사용 판정이 두 벌이 되면 한쪽만 고쳤을 때 두 명령이 다른 파일을 내고,
**그 갈림은 예외가 아니라 조용합니다.**
