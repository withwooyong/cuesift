...
| 사내 자막 공정의 어디에 넣을지 판단해야 한다 | **[docs/OTT_파이프라인_도입검토.md](docs/OTT_파이프라인_도입검토.md)** — 배치 지점·상주 전 보강 항목 |
...
**이득은 예산이 넓을수록 커지고, 어느 예산에서도 전체 Recall이 떨어지지 않습니다.**
아래는 en-ko 벤치마크(TED2020 500건 · `qwen2.5:3b` + `bge-m3`)에서 Tier 0만 쓴 결과와
Tier 1을 켠 결과를 나란히 잰 것입니다.

| 예산 | 의미 반전(`negation`) Recall | **전체 Recall** | 전체 오류 순증 |
|---|---|---|---|
| 5% | 0.00% → 0.00% | 54.6% → 54.6% | +0건 |
| 10% | 1.41% → 1.41% | 73.2% → 73.2% | +0건 |
| 15% | 4.23% → **18.31%** | 85.6% → **87.8%** | **+11건** |
| 20% | 8.45% → **33.80%** | 86.4% → **89.8%** | **+17건** |
| 30% | 19.72% → **42.25%** | 88.0% → **91.2%** | **+16건** |

**예산 10% 이하에서 아무것도 바뀌지 않는 것은 고장이 아닙니다.** Tier 1은 **Tier 0가
아무 신호도 내지 못한 자리**(위험도 0)만 가져가도록 되어 있는데, 예산 10%에서는 컷라인
점수가 0.5라 큐 전체를 Tier 0 신호가 채우고 있어 내어줄 자리가 없습니다. 그 구간에서는
실행이 `Tier 1이 검수 큐를 바꾸지 못했다`라고 경고합니다 — 비용을 내고 아무것도 얻지
못한 것을 조용히 넘기지 않기 위해서입니다. 예산을 15% 이상으로 올리면 컷라인 점수가
0으로 내려가 Tier 1이 볼 자리가 생깁니다.

**이 보호가 없으면 예산 10%에서 전체 Recall이 떨어졌습니다**(73.2% → 72.8%, **-2건**).
Tier 1 후보가 회색지대에서 컷라인까지 올라오면 그만큼이 큐 최하위에서 빠지는데 — 순위는
보존량입니다 — 컷라인 바로 위 구간의 오류 밀도(28%)가 승격 적중률(최대 21%)보다 높아
거래 자체가 손해였습니다.

**ja-ko도 같은 결론입니다.** 다섯 예산에서 전체 순증이 +0 · +0 · +6 · +0 · +12로 어느
칸에서도 음수가 아닙니다 — 두 언어쌍 열 칸을 통틀어 전체 Recall이 떨어지는 칸이 없습니다.
근거: [`bench/results/en-ko-2026-09-13.md`](bench/results/en-ko-2026-09-13.md) ·
[`bench/results/ja-ko-2026-09-13.md`](bench/results/ja-ko-2026-09-13.md) ·
[설계](docs/superpowers/specs/2026-09-13-tier1-pushout-protection-design.md) §3.
...
**임베딩 API 키도 명령줄로 받지 않습니다.** `CUESIFT_EMBED_API_KEY`를 설정하고, 설정하지
않으면 번역용 `CUESIFT_API_KEY`로 폴백합니다. **빈 문자열(`CUESIFT_EMBED_API_KEY=`)로 두면
폴백을 끕니다** — `--embed-base-url`이 번역 엔드포인트와 다른 호스트일 수 있으므로, 무조건
폴백하면 번역용 자격증명이 의도하지 않은 서버의 `Authorization` 헤더에 실립니다.
...
#### Tier 1 역번역 — `--embed-model` (FR-4.2, 동작합니다)

**역번역 신호는 파이프라인 안에서 돕니다.** `--tier1`과 `--embed-model`을 함께 주면
`llm.self_consistency`와 나란히 실행되어 신호별 적발에 `llm.backtranslation`이 실리고
`cost.includes`에 `tier1`이 들어갑니다. 신호 자체도 의미 반전을 정확히 가려냅니다
(negation 세그먼트의 역번역 코사인 0.69 대 0.78, 방향 올바름).

**후보 선정은 극성 표지로 고릅니다.** Tier 0 위험도 순위로만 고르던 초기 구현은 의미
반전을 무작위 위치에 놓아 신호가 볼 기회를 얻지 못했습니다(예산 10% negation Recall
1.41% → 0.00%). 지금은 회색지대 안에서 **부정·이중부정·의무·양보 같은 극성 표지를 가진
세그먼트를 먼저 채웁니다** — 후보 안 의미 반전이 무작위 기대값의 4.37배(예산 10%)·4.91배
(예산 30%)가 되고, negation Recall이 각각 **4.23%·42.25%**로 올랐습니다. 상세:
[`bench/results/en-ko-2026-09-12.md`](bench/results/en-ko-2026-09-12.md).

**그 대가는 위 `--tier1` 절의 표에 있습니다.** 후보가 컷라인까지 올라온 만큼 다른
세그먼트가 밀려나므로, 예산 10%에서는 전체 Recall이 오히려 떨어집니다 — 켜기 전에
그 표를 보십시오.

**신호가 하나도 안 보이면 후보 상한부터 보십시오.** `--tier1-max-ratio`는 비율이고 후보 수는
내림이라, 세그먼트가 적으면 상한이 0이 되어 Tier 1이 아예 돌지 않습니다 — 그때는 위
"후보가 0개일 수 있습니다" 절의 진단 줄이 화면에 나옵니다.
