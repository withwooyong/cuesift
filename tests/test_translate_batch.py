...
def test_id가_정수가_아니면_거부한다() -> None:
    ...
    #
    # 입력이 `"0"`에서 실수로 바뀐 것은 계약이 완화됐기 때문이다 - 정수로
    # 왕복하는 문자열은 이제 받는다(바로 아래 두 테스트). 실수는 그대로
    # 거부다: 0.0을 0으로 접으면 1.5가 어느 세그먼트인지를 반올림이 정하게 된다.
    raw = json.dumps({"translations": [{"id": 0.0, "text": "a"}]})
    with pytest.raises(InvalidResponseError, match="정수"):
        parse_translations(raw, [0])


def test_정수로_왕복하는_문자열_id를_받아들인다() -> None:
    """실측 근거: qwen2.5:3b가 `{"id": "0"}`을 낸다 (2026-08-16 live 실행).

    프롬프트가 `{"id": <번호>}`라고만 써서 타입을 말하지 않았고, 모델은
    문자열을 골랐다. **이 실패 양식에서 폴백은 무력하다** - 타입 습관은
    개별 호출에서도 그대로 재현되므로 호출만 (1+N)배로 늘고 결과가 같다.

    거부하는 쪽이 보수적으로 보이지만 반대다. 번역 실패는 triage를 오염시켜
    실제 검수 비율을 부풀리고, 그 비율이 §9.1 배수의 분모다 (engine.py의
    TranslationResult 독스트링 표: 실패 30건 -> 검수 비율 10%가 15%로).

    `"0"` -> 0은 **추측이 아니다.** 이 모듈이 거부하는 모호성은 "계약을
    만족하는 후보가 둘"이지 같은 값의 표현 차이가 아니다.
    """
    raw = json.dumps({"translations": [{"id": "0", "text": "a"}, {"id": "1", "text": "b"}]})
    assert parse_translations(raw, [0, 1]) == {0: "a", 1: "b"}
...
@pytest.mark.parametrize(
    "item_id",
    [
        "01",  # int는 1인데 표기는 0으로 시작한다. 어느 쪽을 믿을지 정할 근거가 없다
        " 0",  # int()가 앞뒤 공백을 먹는다
        "0.0",  # int()가 ValueError
        "abc",  # int()가 ValueError
        "",  # int()가 ValueError
        "+0",  # int()가 부호를 먹는다
        "1_0",  # **파이썬 int()는 언더스코어를 허용한다.** JSON 숫자에는 없는 문법이다
        "٠",  # **파이썬 int()는 유니코드 십진수를 허용한다.** 아랍-인도 숫자 0
    ],
)
def test_정수로_왕복하지_않는_문자열_id는_거부한다(item_id: str) -> None:
    """완화의 경계는 `str(int(s)) == s` 왕복이다.

    `int()`만으로 완화하면 안 되는 이유가 이 목록이다 - 파이썬의 `int()`는
    JSON 숫자 문법보다 훨씬 넓어서 `"1_0"`(10)과 `"٠"`(0)까지 받는다.
    그것들을 통과시키면 **모델이 낸 적 없는 번호로 접혀** 번역문이 다른
    세그먼트에 붙는데, id 집합 검사는 접힌 뒤의 값만 보므로 잡지 못한다.

    왕복을 걸면 판정이 "int()가 읽어 내는가"가 아니라 "표기가 정규형인가"가
    되어, 위 여덟 개가 전부 여기서 멈춘다.
    """
    raw = json.dumps({"translations": [{"id": item_id, "text": "a"}]})
    with pytest.raises(InvalidResponseError, match="정수"):
        parse_translations(raw, [0])
