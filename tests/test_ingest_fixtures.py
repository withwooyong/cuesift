...
EXPECTED = (
    ...
    "large.srt",
    # WP8b Task 3 - 트리아지 요약의 **신호별 적발**을 재려면 신호가 섞이지 않은
    # 트랙이 필요하다. `large.srt`는 원문이 `안녕하세요 N번째 대사입니다`라 숫자를
    # 담고 있어 `struct.number_missing`이 발화하고, `overlap.vtt`는
    # `spec.overlap`이 발화한다. 여기 큐 10개는 2초씩이고 **겹치지 않으며 숫자를
    # 담지 않는다.**
    #
    # **10개인 것이 계약이다.** `DEFAULT_BATCH_SIZE`가 10이라 정확히 1배치가 되고,
    # 번역 실패를 흉내 내는 `ScriptedProvider` 가짜가 응답 **하나**로 끝난다.
    # 11개가 되면 2배치가 되어 그 가짜가 "대본이 소진됐다"로 죽는다.
    "ten_cues.srt",
)
