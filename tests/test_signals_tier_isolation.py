...
def spy_registered():
    ...
    **tier 0은 남긴다 - 다만 지금 이 파일의 테스트가 그 상태에
    의존하지는 않는다.** 실측: `registry().clear()`로 전부 비우고
    돌려도 지정 파일 기준 통과 개수가 그대로다. tier 1로만 제거 범위를
    좁히는 것은 그래서 **의존성 때문이 아니라 두 가지 다른 이유**다 -
    ① 실제 소비자(`collect_all`)가 tier 0이 존재하는 상태에서 돌아가는
    편이 운영 배선에 더 가깝고, ② 제거 범위를 좁힐수록 fixture가
    손상됐을 때(아래 `finally`가 없다면) 다른 파일까지 번지는 폭발
    반경도 작아진다. (2라운드 리뷰 지적 - 이전 버전은 "테스트들이
    전제한다"고 적었는데 실측과 어긋났다.)
    """
    saved = dict(registry())
    try:
