...
def test_dry_run이_tier1_상한을_말한다(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    계층이 통째로 빠진 화면을 보고 사용자가 실행을 결정한다.

    **이 줄이 채우는 것은 재번역 요청의 상한이다.** 프로바이더 호출 총량은
    재시도·폴백이 얹혀 여전히 아무도 말하지 않는다 - 형제 줄
    (`호출 필요 N개 이상`)이 하한 어법을 쓰는 것과 이 줄이 다른 낱말을 쓰는
    것이 같은 이유다.
    """
...
def test_상한은_올림이_아니라_내림이다(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    **지우면 무엇이 깨지는가 - 이것이 `floor` 계약의 유일한 지지점이다.**
    다른 상한 테스트는 전부 100큐를 쓴다. 그래서 이 함수를 지우거나 큐 수를
    딱 떨어지는 값으로 바꾸는 순간 `floor` -> `ceil` 변이가 **사망 0건**이
    된다 - 결정 실험(리뷰 축2가 두 라운드에서 각각 확인): 이것 **하나만**
    deselect하고 `ceil` 변이를 넣으면 `1266 passed, 0 failed`다. "비슷한
    테스트가 여럿이니 하나쯤 지워도 된다"가 성립하지 않는 자리다.
    """
...
def test_상한이_명시된_값을_그대로_쓴다(
    ...
    **두 낱말은 일부러 다르다 - 일괄로 맞추지 마라.** 앞의 `개별 호출`은
    §12 Q3가 말하는 진짜 프로바이더 호출이고, 화면이 세는 것은 그보다 한 층
    위인 재번역 요청이다. 요청 하나가 재시도·개별 폴백으로 여러 호출이 되므로
    두 수는 같지 않다(`cli._TIER1_BOUND_PREFIX` 주석).
    """
    src = _srt_with_cues(tmp_path / "hundred.srt", 100)
    _patch_provider(monkeypatch, _clean_echo())
