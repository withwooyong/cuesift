...
def test_거대_정수_토큰수는_상한에서_잘린다() -> None:
    ...
    밖이다** - 이 모듈이 전체를 걸어 막고 있는 바로 그 부류의 누수다.

    **NFR-2는 v0.1에서 토큰 사용량만 보고한다 - 통화 환산 비용은 범위
    밖이다**(요구사항정의서 §6 NFR-2, 리뷰로 정정됨). 그래도 이 클램프가
    필요한 이유는 오늘 이 값을 곱하는 코드가 없어서가 아니라, **생기는
    순간 조용히 죽지 않아야 하기 때문이다** - `TokenUsage`는 공개 API이고
    `store/cache.py`가 그대로 JSON에 실어 캐시 파일로 남긴다. 하류가 언젠가
    토큰 x 단가로 비용을 환산하면(v0.2 후보든 사용자 자신의 스크립트든)
    이 값이 그 곱셈의 피연산자가 된다. 아래 단언의 `* 0.15 / 1_000_000`이
    바로 그 연산을 흉내 낸 것이다 - 클램프가 없으면 여기서
    `OverflowError`가 난다.
    """
    body = _ok_body()
