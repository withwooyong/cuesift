...
def test_cli가_실제_프로세스로_동작한다(tmp_path: Path) -> None:
    ...
    **`-m live`로만 돈다.** CI에는 엔드포인트가 없다.
    ...
    # 형식을 어긴 응답도 **일단** 저장한다 - 캐시 계층은 기대 id를 몰라 형식
    # 위반을 판정하지 못하기 때문이고, 지우는 것은 그 위의
    # `translate/engine.py`의 `_discard_cached`다(파킹 #13). 이 테스트는 그
    # 아래층만 보므로 여기서는 저장된 채로 보인다.
    # 그래서 위 단언 하나만으로는 "재개가 쓰레기를 재사용하는 상태"와
