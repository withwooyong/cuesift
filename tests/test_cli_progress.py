...
def test_옵션_이름이_폭_88에서_잘리지_않는다() -> None:
    ...
    **`normalize_rich_message`를 쓰지 않고 `strip_rich_decoration`을 쓴다.**
    더 엄격한 쪽을 골랐다 - 한 줄 안에 이름이 통째로 있는지를 본다.
    "normalize를 쓰면 잘린 것도 통과한다"는 **재현되지 않았다**(리뷰 라운드 2
    실측: COLUMNS 60·70·76·80·84·88·100 + 변이 트리 3종, 두 방식의 판정
    **차이 0건**). rich는 옵션 이름을 줄바꿈으로 접지 않고 `…`로 자르므로
    공백 제거가 도로 붙일 대상이 없기 때문이다. 즉 이 선택은 **무해하지만
    필수는 아니다** - 근거로 삼을 수 있는 것은 "더 엄격하다"까지다.
