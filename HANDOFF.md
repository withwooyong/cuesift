...
**CI 실측치**(PR #10, 3잡 전부 통과): `test 3.11`·`test 3.12`가 각각
**1269 passed / 1 skipped / 3 deselected**, `docs`가 상대 링크 **152개** ·
markdownlint **31 files**.

**로컬과 CI의 pytest 개수는 1건 갈린다** — `data/`가 `.gitignore:77`에 있어 코퍼스가
필요한 테스트 1건이 CI에서 skip된다. **개수가 다른 것이 정상이고, 다르지 않으면
그때가 이상하다.**

**색 조건은 게이트에 없다.** 위 ⑦의 결함은 `FORCE_COLOR=1 pytest`로 로컬에서 잡히는데
표준 게이트는 그것을 안 돈다. **전량을 두 번 돌리는 대신 색 계약이 있는 테스트가 스스로
`FORCE_COLOR`를 켜기로 했다** — 조건이 테스트에 명시되고 게이트 시간이 안 늘어난다.
미래에 생길 같은 결함은 CI가 그물이다(색이 켜진 유일한 환경).
