...
| **Tier 1 토큰 미집계** | ✅ 통로(Task 3·4)에 이어 **배선까지 닫혔다**(Task 6). `--tier1`을 켜고 **Tier 1이 실제로 돈** 실행의 `cost.includes`에 `"tier1"`이 실린다. **판정 기준은 스위치가 아니라 그 계층이 돌았는지다** — 번역이 전량 실패하면 Tier 1이 안 돌아 `["translation"]`만 나온다 |
| **`samples` 상한 부재** | ✅ **Task 5가 코드에 넣었다.** `_TIER1_COST_LIMIT = 0.3`이고 `samples × max_ratio`가 **`0.3`에 닿거나 넘으면** 종료 코드 2로 거부된다 — 경계를 포함한다(`math.isclose`). `samples` 단독 상한은 여전히 두지 않는다(§11 R8: `samples ≤ 10`에는 출처가 없고 이 곱의 출처는 §4다) |
...
**마지막 하나는 "해결"이 아니라 "판정"이다.** 다시 열려면 판정 근거를 반박해야 한다.
