...
| 종료 코드 | **디스크 사정 66 · 내용 결함 70**(기존 상수 재사용, 설계 §8.1의 순서 1번). 최종 리뷰에서 **자막 쓰기도 70으로 통일**했다 — 같은 오염 문자열이 자막·리포트 양쪽으로 흐르므로 어느 쪽이 먼저 죽느냐로 코드가 갈리면 안 된다 |
| 산출물 쓰기 | **원자적이다**(임시 파일 → `os.replace`). 최종 리뷰가 찾았다 — 전에는 실패 시 **기존 정상 파일이 0바이트가 됐다**(실측 1071 → 0). `write_text`는 먼저 truncate하고 그 다음 인코딩한다 |
...
> **이 규칙의 출처는 요구사항정의서 §0.1 하나다.** 사본은 **둘**이다 — 이 문서 ⑤와
> `CHANGELOG.md`의 판정 기준 항목. **고칠 때 이 문서와 §0.1 둘을 함께 고친다.**
>
> **`CHANGELOG.md`는 더 이상 함께 고치지 않는다**(최종 리뷰 D-3·N2). 그것은 **역사 기록**이라
> 과거 항목의 수치를 현재값으로 갈아 끼우면 **이력이 현재 상태를 흉내 내고** 다음 번에 또
> 갈라진다. 대신 그 항목 머리에 "아래 숫자는 그 시점 값이고 현재 값은 §0.1이 낸다"는 표지를
> 달아 **출처이기를 그만두게** 했다. 그 표지를 지우고 수치를 고치지 마라.
>
> 이 문서 ⑤의 사본을 줄이는 것은 문서 구조 결정이라 여전히 후속이다.
...
### 브랜치 전체 리뷰가 이월한 것 — **판정과 함께 남긴다**

리뷰가 **닫지 않기로 판정**한 것들이다. "아직 안 봤다"가 아니라 **"보고 판단했다"** 이므로,
다시 열려면 판정 근거를 먼저 반박해야 한다.

| # | 무엇 | 판정과 근거 |
| --- | --- | --- |
| 1 | **AST 기반 CI 게이트** — `_format_triage_summary`의 산식 재도입을 막는 현재 수단은 CI에 없는 1회성 grep이고 **6형태 중 3형태를 우회**한다(`len(outcome.risks)` · 컴프리헨션 · `len(outcome.selected)`) | **별도 작업.** 오늘 잡을 것이 0이고(그 셋은 **등가 변이**다 — `triaged_segments`가 문자 그대로 `return len(self.risks)`) `.github/` 변경은 이 PR의 **되돌리기 단위를 흐린다.** 프로퍼티 정의가 바뀌는 날 사각이 실재가 된다 |
| 2 | **`cli.py` 크기**(1913줄) | **이 브랜치 책임이 아니다** — AST 문은 410 → 436(**+6.3%**)이고 늘어난 227줄 중 **146줄이 주석**이다. 분할한다면 별도 작업 |
| 3 | `_dry_run_report` 파라미터 **13개** | 지금 문제가 아니다 — 전부 keyword-only이고 같은 파일의 `_translate_one`·`translate`가 **각각 15개**다. 2번과 함께 움직인다 |
| 4 | `segments[].reasons`의 **순서**가 미검증(`reversed` 변이 생존) | NFR-3 재현성 문제. **`signal_hits` 순서는 이번에 닫혔고** 이쪽은 열려 있다 — 같은 종류의 짝이다 |
| 5 | `_COST_INCLUDES`가 실제 집계 범위와 **연동되지 않는다**(문자열 고정값) | **WP8b가 Tier 1을 더하면 갈라진다.** 그때 함께 본다 |
| 6 | `TriageOutcome.__post_init__`의 집합 등호 **한 방향만** 고정 · 중복 id를 막지 않음 | 오늘 그런 입력을 만드는 경로가 없다(실측). 생성 경로가 늘면 다시 본다 |
| 7 | branch coverage 부재 | `--cov-branch`를 실제로 켜 본 결과 **TOTAL 99% 그대로**이고 새로 드러난 둘은 **이 브랜치 밖**이다. `report/`는 branch까지 100% |
| 8 | 66 오류 메시지에 내부 **임시 파일 이름**이 샌다 | 머리에 올바른 대상 경로가 있어 오독 위험 낮음. 게이트 없음 |
| 9 | `write_subtitle`의 `format_=result.format` 의존이 넓어졌다 | 오늘 도달 경로 0. 주석이 의존을 명시하고 `test_확장자_없는_경로에도_원본_포맷으로_쓴다`가 인자를 잠근다 |
| 10 | 새 픽스처에서 `hard(1) + excluded(2) == selected(3)`가 **우연히 성립** | 그 조합을 겨눈 변이를 **스위트는 잡는다**(기함 게이트만 못 잡는다). **우연 없는 픽스처 추적은 무한하다** — 수치 5개면 산술 관계가 여럿이다 |

**이 표가 유일한 기록이다** — 리뷰 산출물은 `.superpowers/`(gitignore) 안에 있어 세션이 끝나면 사라진다.
