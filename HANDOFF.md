...
> Branch: **`feat/tier1-cli`** — **푸시는 돼 있고 PR이 없어 CI가 한 번도 안 돌았다.**
> 아래 "이 브랜치의 배포 상태"를 명령까지 그대로 읽어라. 커밋 수는 매 커밋마다
> 자신을 포함하지 못해 여기 적은 숫자가 항상 낡는다 —
> `git rev-list --count main..HEAD`로 직접 센다(이 문서를 쓴 시점 **39**).
...
## 이 브랜치의 배포 상태 — **네 줄을 따로 읽어라**

**"푸시했다"와 "CI가 돌았다"가 이 저장소에서는 분리돼 있다.** 보통 리포에서는 둘이
같아서 산문으로 쓰면 매번 도로 붙는다 — **이 문단이 두 번 틀렸다**(아래 ⑥). 그래서
표로 가르고 확인 명령을 함께 둔다. 값이 아니라 **명령을 돌려서** 채워라.

| 상태 | 이 문서를 쓴 시점 | 확인 명령 |
| --- | --- | --- |
| 원격에 브랜치가 있나 | ✅ **있다** — `8f1e221`까지 | `git ls-remote origin feat/tier1-cli` |
| 미푸시 커밋 | **17개** | `git rev-list --count origin/feat/tier1-cli..HEAD` |
| PR이 있나 | ❌ **없다**(열린 것도 닫힌 것도) | `gh pr list --head feat/tier1-cli --state all` |
| CI가 돈 적 있나 | ❌ **없다 — PR이 없어서다** | 위 셋의 결과로 판정한다 |

### 다음 행동은 PR이다

```bash
git push                                     # upstream은 이미 origin/feat/tier1-cli다
...
git rev-list --count origin/feat/tier1-cli..HEAD    # 미푸시 커밋
gh pr list --head feat/tier1-cli --state all        # 비면 PR 없음 = CI 미실행
```
