...
| 요구사항정의서 §0.1·§5 머리 | **완료(✅) 판정 기준**과 **상태 열의 적용 범위**를 명시했다. 리뷰 라운드 1이 잡았다 — 아래 "판정 기준이 왜 필요했나" |
| `docs/WBS.md` | FR-6.3 CLI의 담당을 WP6으로 명시, 의존 그래프에 `FR-6.3 CLI → FR-4.3 CLI` 라벨, 완료 개수 32 → 30, "남은 1순위는 WP8b 하나다" 서술 정정 |
| `CHANGELOG.md` | `[Unreleased]`의 Added 4건 · Changed 6건 |
| `HANDOFF.md` | 이 문서 |
...
이제 요구사항정의서 §0.1에 규칙이 있다 — **FR의 핵심 동사가 요구하는 층**이고, 어느
층인지는 **목적절의 수혜자**가 가른다.

| 목적절의 수혜자 | 핵심 동사 | 충족되는 층 |
| --- | --- | --- |
| **파이프라인 안** (다음 단계 · 라이브러리를 직접 호출하는 개발자) | "산출한다" · "포함한다" · "정의한다" · "측정한다" | 라이브러리 동작 |
| **라이브러리 밖의 사람** (검수자 · CLI 사용자 · CI) | "설정할 수 있다" · "지정할 수 있다" · "덮어쓸 수 있다" · "받는다" · "출력한다" · "반환한다" · **"…할 수 있게 한다"** | 사용자 통로(CLI·설정 파일·리포트)까지 |

아래 행이 곧 "미완료"는 아니다 — FR-7.1·FR-7.5·FR-5.3은 통로가 실제로 있어서 ✅다.

**어미가 아니라 수혜자의 위치가 가른다.** FR-6.5의 "QE 모델을 꽂을 수 **있게 한다**"는
같은 어미지만 수혜자가 라이브러리를 직접 호출하는 개발자라 통로가 라이브러리 자신이고,
그래서 ✅다. FR-6.4의 검수자는 라이브러리 안에 도달할 수단이 없다.

수식어로 붙은 속성(FR-6.1의 "**설정 가능한** 가중치")은 별도 FR로 분리되지 않는 한 판정을
좌우하지 않되, **미구현 통로는 상태 칸에 명시한다** — 판정을 바꾸지 않는 것과 감추는 것은
다르다.

**규칙 초판에는 구멍이 있었고 재리뷰가 잡았다.** 동사 목록만 적었더니 FR-6.4의
"기록한다"가 라이브러리 동사로 읽혀 규칙은 ✅를 내는데 표기는 🟡였다 — F1이 닫으려던
"잣대가 두 개"가 FR-6.1/FR-6.4 쌍에서 재현된 것이다. **동사 목록보다 수혜자가 먼저다**:
FR-6.4의 목적은 기록 자체가 아니라 **사람이 설명을 받는 것**이고, 그래서 통로(FR-7.2
`review.json`)가 완료 조건이다.

**개수가 틀리는 방향은 둘이다** — 수식어 규칙이 없으면 FR-6.1을 내려 **29**, 수혜자
규칙이 없으면 FR-6.4를 올려 **31**. 둘이 함께 있어야 30이 유지된다.

**규칙을 고치는 사람은 상태 열이 있는 FR 13개에 전수 대조하라.** 이 규칙은 두 번 고쳐졌고
**두 번 다 대조가 구멍을 찾았다** — 1판은 FR-6.4, 2판은 FR-6.5에서 어긋났다. 대조 없이
규칙만 읽으면 둘 다 그럴듯하게 보인다.
