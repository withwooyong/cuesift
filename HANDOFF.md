...
> Branch: **`feat/tier1-signals`** — 커밋 20개, `main`에 아직 안 올라갔다.
...
닫혔다(`9d57e9a`, Ruling P13, 회귀 테스트 `test_전역_index_대신_로컬_0으로_보낸다` 포함,
...
닫혔지만 같은 유형이 다른 프롬프트(예: 다른 리터럴 예시를 쓰는 자리)에도 있을 수 있다.
아래 ④(`_run_single`)가 같은 클래스의 **미검증** 자리다.
...
`fuse.py:92`·`:118`이 "가중 평균 시절"이라 회고한다)과 "가중치 8종"(실제는 WP8a가
`llm.self_consistency`를 더해 **10종**)이었다. `tier1.py`의 2라운드 설계 전체가 noisy-or의
단조성 위에 서 있고 리뷰어가 회색지대 세그먼트 점수가 0.7500 → 0.7694로 **올라** 컷라인을
넘는 것을 실측으로 확인했는데, CHANGELOG를 먼저 읽은 사람은 정반대(가중 평균)를 들었을
것이다 — Keep a Changelog의 `[Unreleased]`는 현재 상태를 서술해야 하므로 바로잡았다.
...
### ① WP8b 착수 전 정리해야 할 것 3건 (실측만 했다, 구현은 안 했다)

| 항목 | 실측 | 왜 지금 안 고쳤나 |
| --- | --- | --- |
| **배치 무력화로 호출 10배** (가장 중요) | 세그먼트 10개 × samples=3에서 **30회**(배치면 3회) — 호출 배수는 재현됨. **문자 수는 여기 적지 않는다**(입력 문장 의존이라 재현마다 갈라진다 — Task 7 Item ⑧, 재리뷰가 실제로 다른 값을 얻었다. 정확한 값은 `signals/llm.py`의 `_retranslate` 독스트링이 단일 출처) | 설계 §4.1의 세그먼트 단위 프로토콜(`collect_tier1(seg, ctx)`)이 원인. 프로토콜 변경은 Task 2·6을 함께 건드리는 설계 변경이고 WP8a는 라이브러리 계층까지가 범위다. **CLI를 배선하는 사람이 이것을 모르면 실제 트랙에서 10배 비용이 그대로 난다** |
| **Tier 1 토큰 사용량 미집계** | `collect_tier1`이 `TranslationResult.usage`를 올려 보낼 통로가 없다 | NFR-2·FR-7.4의 비용 숫자에서 **가장 비싼 계층만 빠진다.** 누적은 리포트 계층(WP5 나머지·WP8b)의 일 |
...
### ④ `_run_single`(`engine.py`)에도 같은 패턴이 있다 — **코드는 확인, 실물은 미확인**
...
**코드 경로 취약성은 실측됐다**(위 인용이 `engine.py:384` 그대로다). **실물 모델이 그
폴백 프롬프트에서도 같게(index != 0일 때 id를 0으로 답하는 식으로) 행동하는지는
검증하지 않았다** — WP8a 범위 밖이라 확인하지 않았고, 추측을 사실로 적지 않는다. 다음
사람이 `qwen2.5:3b`류 로컬 모델로 다중 세그먼트 배치의 개별 폴백을 실물 검증할 때
이 자리를 먼저 본다.
...
| **P13 수정은 내가 하지 않았다 — 권한 밖이었고, 실제로도 다른 에이전트가 처리했다** | Task 7의 쓰기 범위는 문서와 live 테스트뿐이다(`src/` 금지). 실측 근거를 갖춰 보고하는 것까지가 이 태스크의 책임이었다 |
...
| `pytest --cov=cuesift --cov-report=term-missing` | **1049 passed, 3 deselected** · 커버리지 **99%**(1655문 중 22 미실행) |
| `pytest -m live -v -s` (Ollama `qwen2.5:3b`) | **3 passed, 1049 deselected** in 19.2초 — `llm.self_consistency` 발화 확인, `warn` 메시지 없음(Tier 1 정상 실행) |
