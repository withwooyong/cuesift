...
> **구현 완료 — 실제 코드는 위 스케치보다 넓다.** 리뷰가 실측으로 찾은 것을 3라운드에 걸쳐
> 반영했으므로 `src/cuesift/cli.py`의 `_resolve_profile`과 `src/cuesift/spec/profile.py`를
> 직접 보라. 위 코드 블록은 착수 시점의 설계 스케치다. 주요 차이 넷:
>
> | 항목 | 위 스케치 | 실제 구현 |
> | --- | --- | --- |
> | 예외 처리 | `cli.py`가 `(OSError, ValueError)`를 **열거** | **`spec/profile.py`가 실패를 `ValueError`로 정규화**하고 `cli.py`는 열거하지 않는다 |
> | 확장자 판별 | `spec.endswith(...)` | `spec.lower().endswith(...)` — 대문자 `.YAML`이 D10의 틀린 진단을 받았다 |
> | 반환 | `SpecProfile` | **`(SpecProfile, str)`** — 라벨이 없으면 사용자 프로파일이 내장과 출력에서 구분되지 않는다 |
> | 검증 범위 | 없음 | `profile.py`가 타입·`bool`·`NaN`/`inf`·거대 정수를 거부한다 |
>
> **왜 이렇게까지 갔나:** 종료 코드가 계약인 저장소에서 `_resolve_profile`이 예외를 흘리면
> typer가 exit 1을 내고, 이 저장소에서 1은 **"규격 위반 발견"** 이다. 즉 프로파일 설정 사고가
> **자막 결함으로 오보**되어 사용자가 멀쩡한 자막을 고치려 든다. 실측으로 닫은 누수가 12종이다.
...
- Produces:
  ...
  - `_format_report(*, source_name, fmt, profile_label, cue_total, violations, event_index) -> list[str]`
...
def _format_report(
    ...
    **순수 함수인 것이 요점이다.** `CliRunner` 없이 문자열 입출력으로 직접
    ...
    """
    # `검사 큐`인 것이 중요하다. `cue_total`은 **필터 후** 세그먼트 수인데 큐 번호는
    # 원본 기준이라, 주석이 있는 파일에서는 "큐 2개" 아래 `#4`가 찍혀 모순으로 읽힌다.
    # 분모를 원본 이벤트 수로 바꾸면 안 된다 — 주석·드로잉은 검사 대상이 아니므로
    # 비율이 과소평가되고 그것은 이 프로젝트의 핵심 지표를 건드린다.
...
def test_an_unreadable_file_exits_66_not_1(monkeypatch):
    ...
    **권한을 실제로 조작하지 않는 이유:** `chmod 000`은 Windows에서 읽기를 막지 못하고
    (읽기 전용 플래그만 선다), 배타 잠금은 별도 프로세스가 필요하다. 이 저장소는
    Windows에서 개발하고 CI는 Linux이므로 어느 쪽도 양쪽에서 같게 동작하지 않는다.
...
- [ ] **Step 4: 인제스트가 `OSError`를 `IngestError`로 정규화하게 한다**

Task 5 리뷰가 실측한 것이다. **읽을 수 없는 파일이 exit 66이 아니라 exit 1로 나간다.**

```text
# 다른 프로세스가 배타 잠금한 정상 SRT
exit code: 1    <-- 기대 66
stderr: PermissionError: [Errno 13] Permission denied: 'locked.srt'
```

`src/cuesift/ingest/loader.py`의 `except (Pysubs2Error, ValueError)`가 **`OSError` 계열을
안 잡고**, `_reject_non_subtitle`의 `path.is_file()`은 존재만 보고 읽기 권한은 보지 않는다.
그래서 `PermissionError`가 `load_subtitle`을 통과해 `check()`의 `except IngestError`도 통과하고,
미처리 traceback이 **exit 1 = "규격 위반 발견"** 이 된다.

**Windows 실사용에서 흔하다** — 편집기·트랜스코더·OneDrive 동기화가 자막을 잡고 있다.
Linux CI에서는 mode 000 파일이나 `+x` 없는 디렉터리가 같은 결과다.

**`loader.py`에서 정규화하라.** Task 4가 `spec/profile.py`에 세운 선례와 같다 —
호출자가 피호출자의 예외를 열거하는 방식은 원리적으로 뒤처지므로, 피호출자가 자기 실패를
하나의 타입으로 모은다. `reason`은 새 값(`unreadable` 등)을 쓰고, 기존 6종
(`empty`·`decode`·`parse`·`not_found`·`video_input`·`bad_timecode`)과 겹치지 않게 하라.

**회귀 테스트는 실제로 읽을 수 없는 파일로 실패를 먼저 확인해야 한다.** Windows에서는
배타 잠금(`msvcrt.locking` 또는 별도 프로세스), POSIX에서는 `chmod 000`이다.
**두 플랫폼에서 같은 방식이 안 되면 그 사실을 보고하라** — CI는 Linux이고 개발은 Windows다.
...
def check(
    ...
    **신호 엔진을 통과하지 않는다**(설계 D3). `collect_all`→`fuse`→`triage`가
    ...
    # 출력에는 사용자가 준 파일 경로와 --spec 경로가 그대로 실린다. Windows 기본 로케일
    # (cp949)에서 인코딩할 수 없는 문자가 그 안에 있으면 리다이렉트 시 UnicodeEncodeError로
    # 프로세스가 죽고 종료 코드 1이 나간다 — 이 저장소에서 1은 "규격 위반 발견"이라
    # **위반 0건인 깨끗한 파일이 CI에서 실패로 읽힌다.** 실측된 사례:
    # `Amélie.srt`(U+00E9) · `S01E01 – ko.srt`(U+2013). 이모지·간체 한자·NBSP도 같다.
    # em dash 금지는 우리가 쓰는 리터럴만 통제하고 사용자 입력이 흐르는 이 경로는 못 막는다.
...
- `--spec`이 `.yaml`/`.yml` 경로를 받는다 — FR-5.3이 CLI에서 도달 가능해졌다.
  확장자는 대소문자를 가리지 않는다
- 규격 프로파일 로더가 **잘못된 내용을 전부 `ValueError`로 정규화한다.** 값 타입 오류·
  `bool`·`NaN`·`inf`·표현 범위를 넘는 정수·YAML 문법 오류를 거부한다

### Fixed
- 프로파일 파일이 잘못됐을 때 **종료 코드 1**(= "규격 위반 발견")로 오보되던 것을 고쳤다.
  설정 사고가 자막 결함으로 읽혀 사용자가 멀쩡한 자막을 고치려 들던 문제다 —
  실측으로 12종의 누수 경로를 닫았다
- `max_cps: .nan`처럼 비유한 값이 로드에 성공해 **CPS 위반이 영원히 발화하지 않던** 문제.
  NaN과의 모든 비교가 False라 증상 없이 신호만 죽었다
...
> **❌ 뒤집힘 — 위 판단은 거짓 전제 위에 있었다** (브랜치 최종 리뷰, 2026-08-13).
> **`--config` 옵션은 `cli.py`의 그룹 콜백에 처음부터 있었다.** 위 근거가 든 실측
> `cuesift check x.srt --config c.yaml` → exit 2는 **그 자체로는 맞지만** 옵션이 없는
> 위치에서만 시험한 것이다 — `--config`는 그룹 옵션이라 **서브커맨드 앞**에 와야 하고,
> 거기서는 조용히 성공한다.
>
> | 호출 | 실측 | 무엇을 보여주나 |
> | --- | --- | --- |
> | `check x.srt --spec ko --config c.yaml` | exit **2** | 위 근거가 본 것. 서브커맨드에는 이 옵션이 없다 |
> | `--config c.yaml check x.srt --spec ko` | exit **0**, 아무 출력 없음 | **옵션이 실재하고 조용히 무시됐다** |
>
> 게다가 `--help`가 **거짓을 현재형으로 약속**하고 있었다("기본: `./cuesift.yaml`" ·
> "CLI 인자가 설정 파일보다 우선합니다"). 둘 다 사실이 아니다 — `./cuesift.yaml`은
> 읽히지 않고 우선순위 해결은 존재하지 않는다. **사용자가 규격을 덮어썼다고 믿는데
> 내장 프로파일로 검사되고 exit 0으로 통과한다.** 설계 D12가 적은 근거("조용한 무시는
> 이 저장소의 규율에 어긋난다")가 정확히 그대로 재현된 것이다.
>
> **설계 D12대로 구현했다** — 그룹 콜백에서 stderr 경고, `--help` 문구를 미래형으로,
> 회귀 테스트 2건. 설계 §7.1 스트림 표의 "`--config` 미지원 경고 | stderr" 줄과
> D12 행은 **이제 참이 되었으므로 각주가 필요 없다.**
>
> **교훈: "옵션이 없다"를 한 위치에서만 확인했다.** 이 세션이 세 번 반복한 형태다 —
> 주석·계획서의 "무엇이 깨지는가"는 주장이고, 주장은 테스트와 같은 기준으로 검증해야 한다.
