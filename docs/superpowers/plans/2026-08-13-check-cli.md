...
def test_check_track_keeps_both_when_a_cue_is_empty_and_overlapping():
    ...
    kinds = [tv.violation.kind for tv in check_track(segments, profile) if tv.segment_id == "00001"]
...
**`test_check_track_keeps_both_when_a_cue_is_empty_and_overlapping`은 반드시 실패시켜 봐라.**
`check_track` 안에서 두 dict를 `{**overlaps, **empties}`로 합치도록 잠시 바꾸면 실패해야 한다.
실패하지 않으면 그 테스트는 아무것도 지키지 않는다. 확인한 뒤 원상복구하라.
...
한글 한 글자가 2폭이므로 아래 코드 블록이 그대로 통과하지 않을 수 있다.
**`ruff format`의 정규 래핑에 맡기고, 그 결과가 테스트 기대 문자열을 바꾸지 않는지 확인하라.**
