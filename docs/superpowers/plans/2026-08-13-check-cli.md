...
매번 읽어라. **착수 시점 316건**이고, 태스크마다 늘어야 한다. 줄거나 그대로면 테스트가
수집되지 않은 것이다.
...
def test_check_track_orders_violations_by_segment_then_by_kind():
    ...
    segments = [
        ...
        Segment(id="00000", index=0, start_ms=0, end_ms=3000, source_text="첫 줄\n둘째 줄\n셋째 줄"),
...
```bash
.venv/Scripts/python.exe -m pytest tests/test_spec_check.py -k track -v
```

기대: 4건 전부 `ImportError`/`NameError`로 FAIL.

- [ ] **Step 3: `TrackViolation`과 `check_track`을 구현한다**
...
기대: 신규 4건 PASS. **`measured == 22.0`과 `25.5`가 정확히 맞는지 확인하라** —
...
def test_resolve_profile_reads_a_yaml_path():
    ...
    profile = _resolve_profile("specs/ted-ko.yaml")
...
```python
from cuesift.cli import _format_report, _format_timecode, _resolve_profile, app
...
`tests/test_cli.py:38-41`의 `test_check_accepts_documented_flags`는 **배선 후 반드시 깨진다** —
...
```bash
.venv/Scripts/python.exe -m cuesift check tests/fixtures/ingest/check_violations.ass --spec ko
echo "exit=$?"
```
...
수집 개수 **333 → 348** 안팎(`parametrize` 4건 포함).
...
**이것은 사용자가 승인한 설계를 계획 단계에서 뒤집은 유일한 항목이므로, 실행 전에 확인이 필요하다.**
