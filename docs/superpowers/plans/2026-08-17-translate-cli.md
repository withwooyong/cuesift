...
**이 절이 본문보다 최신이다**(저장소 규약, `CLAUDE.md`). 본문은 착수 시점 판이라 아래와
어긋나는 자리가 있고, 그때는 **여기를 따른다.**

| 무엇 | 착수 시점 계획 | 지금 | 왜 |
| --- | --- | --- | --- |
| **형식 위반 응답의 캐시**<br>(Task 10 Step 4의 README 문안) | 형식을 어긴 응답도 캐시된다. 재실행으로는 나아지지 않으니 모델을 바꾸거나 `--no-cache`를 쓴다 | **`invalid_response`는 캐시에서 폐기된다.** 재실행하면 그 세그먼트만 다시 불린다 | 2026-08-29 [파싱 실패 응답 폐기](2026-08-29-cache-discard-invalid.md)(파킹 #13)가 설계 결정 U10을 뒤집었다. 캐시된 실패가 **재실행을 실제 호출 0개로 만들어** 가장 흔한 복구 수단을 무력화하고 있었다 |

**`empty_translation`은 지금도 캐시에 남는다** — 계약을 지킨 응답이라 폐기하면 같은
배치에서 성공한 나머지까지 다시 결제한다. 현행 문안은 [README](../../../README.md)와
[사용법](../../사용법.md)에 있다.

**Task 10 Step 4의 문안만은 사료로 두지 않고 고쳤다.** 그것은 기록이 아니라 **README에
그대로 삽입하라는 지시**라서, 낡은 채로 두면 다음 사람이 폐기된 문안을 README에 다시
심는다. 문체는 착수 시점 판(`~다`)을 유지했다 — 지금 README는 `~습니다`로 통일돼 있어
그대로 붙이지 않고 옮겨 적어야 한다.
...
**형식을 어긴 응답은 캐시에 남지 않는다.** 파싱조차 되지 않은 응답
(`invalid_response`)은 지워지므로, 같은 명령을 다시 치면 그 배치만 실제로
다시 호출된다.

**빈 번역(`empty_translation`)은 캐시에 남는다.** 개수도 번호도 맞은, 계약을
지킨 응답이기 때문이다 — 이쪽은 모델을 바꾸거나 `--no-cache`를 쓰거나
`.cuesift/cache/`를 지운다.
