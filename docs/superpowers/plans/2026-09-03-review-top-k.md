...
**이 절은 계획 수립 시점의 정정이다.** 구현하면서 뒤집힌 것은 아래
[구현 중 바뀐 결정](#구현-중-바뀐-결정)에 여섯 건이 더 있고, **그쪽이 더 최신이다.**
...
- [x] **단계 4: README와 CHANGELOG**

README의 옵션 표에 `--review-top-k`를 더한다. CHANGELOG는 Keep a Changelog 형식으로
`Added`에 적는다(`policy_value` 타입 확장은 `Changed`다).

> **실측 정정** - `CHANGELOG.md`도 **LF다.** 실측: `CRLF 0 · LF 356`.
> 그래도 파이썬으로 고칠 때는 `newline=""`을 주는 편이 안전하다 - 기본값은 쓰기 시점에
> `os.linesep`으로 번역해 Windows에서 파일 전체를 CRLF로 바꿔 놓는다.
...
**이 절이 본문 코드 블록보다 최신이다.** 구현하면서 뒤집힌 것들이라, 계획서를 나중에
읽는 사람은 본문보다 이 표를 먼저 봐야 한다(리포 규약).

| # | 태스크 | 계획서가 적은 것 | 실제로 한 것 | 왜 갈렸나 |
| --- | --- | --- | --- | --- |
| 1 | Task 2 | 단계 6의 변이(`_resolve_exclusive`의 양보를 뒤집기)가 기존 회귀에서 잡힌다 | **잡히지 않았다.** 기존 회귀 4건이 `exit_code == 0`만 단언해 **양보가 뒤집혀도 관측되지 않는 구멍**이었다. 그물을 보강한 뒤에야 캐시·예산 두 자리에서 각각 1건 FAIL을 확인했다 | `exit 0`은 "양보가 옳게 일어났다"를 말하지 않는다. **종료 코드만 보는 회귀는 통과하는 게이트가 아니라 없는 게이트다** |
| 2 | Task 2 | (언급 없음) | 자막/영상 자리의 `if "input" in losers:` 분기는 **도달 불가능한 죽은 코드다** - 위치 인자는 `default_map`에 실리지 않는다. **이번 범위 밖이라 고치지 않았다** | 설정 축과 명령줄 축을 같은 함수가 다루는데, 위치 인자는 설정 축에 애초에 들어오지 못한다 |
| 3 | Task 4 | 단계 1 코드 블록이 `payload["policy"]`를 읽는다 | **`payload["summary"]["policy"]`다.** 계획서대로 쓰면 `KeyError`다 | 계획서가 요구사항정의서 §8.4의 JSON 예시를 눈으로 옮기며 한 겹을 빠뜨렸다. `TriageOutcome`은 평평하지만 **직렬화 결과는 `summary` 아래로 한 겹 들어간다** |
| 4 | Task 4 | 지시한 변이 1회로 `isinstance` 그물이 사는지 증명된다 | **증명되지 않는다.** 크래시가 먼저 나서 그물이 잡았는지 크래시가 잡았는지 갈리지 않는다. **직렬화 경계에서 한 번 더 깨뜨려야** 증명된다 | 변이가 그물보다 **앞 단계**를 죽이면 그물은 실행되지 않는다. 고른 변이 지점이 그물과 같은 층인지를 먼저 본다 |
| 5 | Task 5 | 단계 2의 테스트가 `--dry-run`으로 설정 키를 확인한다 | **`--dry-run`은 트리아지를 아예 호출하지 않아 `policy_label`이 출력되지 않는다.** 실제 선별을 도는 헬퍼로 바꿨다 | `--dry-run`은 호출 수만 세는 경로다. **설정 키가 도달하는지를 보려면 그 키를 소비하는 코드까지 실제로 돌아야 한다** |
| 6 | Task 6 | `CHANGELOG.md`는 CRLF, 나머지는 LF | **`CHANGELOG.md`도 LF다**(실측 `CRLF 0 · LF 356`) | 과거 어느 시점에 정규화된 것으로 보인다. 원인은 확인하지 않았다 |

**1·4·5가 같은 부류다 - 게이트가 자기가 재려는 것을 재지 못했다.** 셋 다 "통과했나"가
아니라 **"무엇을 대상으로 통과했나"** 를 물어야 드러나는 자리이고, 이 리포가 1급으로
경계하는 실패 모양이다. 3은 파생 문서를 눈으로 옮긴 자리이고, 2는 범위 밖의 발견이며,
6은 계획서가 확인 없이 적은 환경 사실이다.

## 완료 조건

**아래는 실측값이다.** 착수 값은 계획 수립 때 잰 것이고, 완료 값은 Task 6 단계 6에서
다섯 게이트를 실제로 돌려 읽은 것이다.

| 게이트 | 착수 | 완료 (실측) |
| --- | --- | --- |
| `pytest` | 1743 passed · 5 deselected | **1769 passed · 0 failed · 5 deselected** (신규 **26**건: 11+2+**8**+1+4) |
...
| `scripts/check_links.py` | 마크다운 **45개** · 상대 링크 **240개** · 깨진 링크 0 | 마크다운 **45개** · 깨진 링크 0 |
| `npx markdownlint-cli2` | Linting: **45 files** · 0 issues | Linting: **45 files** · 0 issues |
| FR 완료 개수 | 40 / 42 | **41 / 42** |

**이 표는 계획 수립 시점의 예상값에서 세 자리가 어긋나 정정한 것이다.**

| 항목 | 계획서가 예상한 값 | 실측 | 왜 어긋났나 |
| --- | --- | --- | --- |
| 완료 pytest | 1768 passed (신규 25) | **1769 passed** (신규 26) | Task 3의 신규가 7건이 아니라 **8건**이었다 |
| 착수 마크다운 | 44개 · 상대 링크 238개 | **45개 · 240개** | **계획서가 자기 자신을 세지 못했다.** 계획서와 스펙이 `3381f6b`으로 들어가며 45개가 됐다 |
| 완료 마크다운 | 45개(하나 는다) | **45개 유지** | 같은 이유. Task 6은 새 문서를 만들지 않고 기존 문서를 고친다 |

**착수 값을 자기 자신이 커밋되기 전에 재면 반드시 어긋난다** - 계획서가 커밋되는 순간
마크다운 개수가 오르고, 그 계획서 안의 "착수 값"은 그때 이미 낡는다.
...
**Task 6(문서)은 단독으로 되돌려도 코드는 깨지지 않지만**, 되돌리면 요구사항정의서 §5.6이
FR-6.3을 🟡로 되돌려 **구현과 판정이 갈린다** - 코드를 함께 되돌리지 않을 거라면 이 커밋도
되돌리지 않는다.
