...
기대: **5건 중 3건 FAIL, 2건 PASS.** 필터가 없으므로 `tags.ass`가 4개 세그먼트를 내고
`all_comments.ass`는 예외 없이 2개를 낸다.

구현 전에 이미 통과하는 둘은 아래와 같고, **정상이다.**

| 테스트 | 왜 구현 전에도 통과하나 |
| --- | --- |
| `test_empty_text_cue_is_preserved` | 빈 텍스트 큐는 애초에 걸러지지 않는다 |
| `test_filtered_events_survive_in_subs_for_roundtrip` | **불변조건 테스트**다 — 원본 `subs`가 온전한지만 보므로 필터 유무와 무관하다 |

두 번째가 값을 하지 않는다는 뜻이 아니다. 이 테스트를 실패시키는 변경이 둘 실재한다 —
`_keep_displayed`가 `subs.events`를 **in-place로 잘라내는** 구현, 그리고 **필터된 사본을
`IngestResult.subs`에 담고 `event_index`를 사본 기준으로 재계산**하는 구현이다.
후자는 `test_event_index_maps_to_the_right_original_event`의 루프 단언을 **통과시키므로**
(사본 기준으로 대응이 일관되기 때문) `len(result.subs) == 4`만이 잡는다.
...
**이 테스트는 초판에서 한 번 바뀌었다.** 초판은
`test_comment_line_would_be_spec_checked_if_kept`라는 이름으로 세그먼트 제외 여부만
확인했는데, 리뷰가 **이름이 약속한 것("규격 검사를 받는다")을 몸체가 검증하지 않는다**고
지적했다. 실측해 보니 주석 줄은 애초에 규격 위반을 내지 않아 그 이름은 이 픽스처로
증명할 수 없었다. **결론이 아니라 해악의 정체가 틀렸던 것이다** — 주석의 해악은
규격 위반이 아니라 개수다.
