...
**남은 사각(문서화하고 수용).** `len(outcome.risks)`는 두 게이트 어느 쪽도 잡지 못한다 —
오늘은 등가라 잡을 것이 없고, `triaged_segments`의 정의가 바뀌면 Step 7b의 패턴에도
매치되지 않아 미래의 갈라짐도 놓친다. 거기 서 있는 것은 `models.py:91-93`의 독스트링
("두 값이 지금 같다는 것은 우연이지 보장이 아니다")뿐이다. 게이트를 CI에 넣는 것이 진짜
해법이고 이 브랜치 범위 밖이다 — **브랜치 전체 리뷰가 판단한다.**
