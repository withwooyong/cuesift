...
**즉 이 게이트가 막겠다고 한 상태(화면과 파일이 각자 세는 것)가 게이트와 스위트를 둘 다 빠져나간다.** 반대 방향 오탐도 있다 — 함수 범위 안 주석에 `sorted(`를 글자 그대로 쓰면 자기 게이트에 걸린다(Task 4 구현자의 초안이 실제로 그랬다).

**따라서 이것은 보조 게이트다.** 흔한 형태를 값싸게 훑을 뿐이고, "산식이 한 벌이다"의 진짜 방어선은 **Task 6의 `test_화면_요약과_파일_수치가_일치한다`**다 — 거기서는 두 출력이 한 실행에 함께 존재하므로 값을 직접 비교할 수 있다. Step 7b가 통과했다고 D8이 지켜졌다고 결론 내리지 마라.

이 게이트는 `.github/`·`scripts/` 어디에도 없는 1회성 수동 명령이라 회귀도 막지 못한다.
