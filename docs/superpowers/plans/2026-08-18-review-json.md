...
- [ ] **Step 7b: 산식 재도입의 흔한 형태를 훑는다 — 보조 게이트다**
...
**범위가 0줄이 아닌지 함께 확인하라** — 빈 범위도 "0건"을 낸다. `sed` 출력의 줄 수를 세어 함수 본문이 실제로 잡혔는지 본다.

**이 게이트는 AST가 아니라 텍스트를 본다. 그리고 자기 목적을 재지 못한다** — Task 4 리뷰가 실측했다.

| 산식 재도입 변이 | Step 7b | 전 스위트 |
| --- | --- | --- |
| `total = len(outcome.risks)` | **우회** — 패턴이 `len(risks)`라 매치되지 않는다 | **1128 passed — 생존** |
| `selected = len([r for r in outcome.risks if r.selected])` | **우회** — 컴프리헨션이라 `sum(`이 없다 | **1128 passed — 생존** |

**이 표의 "생존"은 게이트 부실의 증거가 아니다 — Task 6이 그 해석을 뒤집었다.** 두 형태는
`TriageOutcome`의 프로퍼티 정의와 **글자 그대로 같은 계산**이다.

| 프로퍼티 | 정의 | 위치 |
| --- | --- | --- |
| `triaged_segments` | `return len(self.risks)` | `report/models.py:85` |
| `selected_for_review` | `return len(self.selected)` (= `tuple(r for r in self.risks if r.selected)`) | `report/models.py:100`·`:104` |

즉 **등가 변이(equivalent mutant)** 이고, 어떤 테스트도 이 둘을 구별할 수 없다 — 구별할 필요도
없다. 오늘의 동작이 동일하기 때문이다. 위 측정은 옳았고 **해석이 틀렸다**: "게이트가 못 잡는다"가
아니라 "잡을 결함이 아니다"였다. 변이 테스트에서 생존은 두 가지를 뜻한다 — 테스트가 부실하거나,
변이가 등가이거나. 둘을 가르려면 **변이 대상의 정의를 읽어야 한다.**

반대 방향 오탐도 있다 — 함수 범위 안 주석에 `sorted(`를 글자 그대로 쓰면 자기 게이트에 걸린다
(Task 4 구현자의 초안이 실제로 그랬다).

**따라서 이것은 보조 게이트다.** 다만 이유는 "Task 6 테스트가 대신 잡아서"가 **아니다.**
이 게이트는 `.github/`·`scripts/` 어디에도 없는 1회성 수동 명령이라 **회귀를 막지 못하고**,
AST가 아니라 텍스트를 보므로 사각이 남는다. 그것이 보조인 이유다.

**두 게이트는 경쟁이 아니라 보완 관계다.**

| 게이트 | 실제로 막는 것 |
| --- | --- |
| Step 7b grep | **미래의** 갈라짐 — 프로퍼티 정의가 바뀌면 굳어 있는 사본이 조용히 어긋난다 (단, 아래 사각 제외) |
| Task 6 `test_화면_요약과_파일_수치가_일치한다` | **오늘의** 갈라짐 — 화면이 **실제로 다른 값**을 찍는 것 |

후자는 Task 6에서 실증됐다 — M5(`triaged` 자리에 `total_segments`) · M14(`selected` 자리에
`triaged`) · M15(`hard fail` 자리에 `selected`) 셋 다 이 테스트가 격추했고 **살해자 이름으로**
확인했다.

**그러나 그 셋만으로는 이 게이트의 한계 가치(marginal value)를 증명하지 못했다.** Task 6 게이트
리뷰가 변이 22종을 독립 실행해 실측한 결과, 이 테스트가 격추한 9종(M5·M14·M15·M16·M17·M18·
F1·F2·F3) **전부를 기존 `test_cli_triage.py`/`test_report_json.py`가 함께 격추**했다 —
**단독 격추 0건**이었다. 리뷰어가 기존 픽스처에서 값이 우연히 일치해 놓칠 법한 형태를 일부러
셋 만들었는데(M16 `selected`←`hard` · M17 `hard`←`triaged` · M18 `total`←`selected`) 셋 다
기존 테스트가 먼저 잡았다.

**원인은 픽스처 대칭이었다.** `_blank_at({2,5,9}, 10)`에서는 `hard_fail_count == 0`이고, 화면
hard fail을 단언하는 자리가 스위트 전체에 둘뿐인데 **둘 다 값이 0**이었다 — 대조가 `0 == 0`이라
`hard = 0` 변이가 전 스위트를 통과했다.

**fix 라운드 1이 이것을 닫았다.** 픽스처를 `scripted_at({0: 한글 3줄, 2: 공백, 5: 공백}, 10)` ·
예산 30%로 바꿔 **다섯 수치가 전부 다르게** 만들었다(total 10 · triaged 8 · excluded 2 ·
selected 3 · hard 1). 그 뒤 `hard = 0` 변이는 이 테스트가 **단독으로** 격추한다.
같은 라운드에서 `signal_hits` 순서 변이도 `test_신호별_적발_집계가_화면과_일치한다`의
**단독 격추**가 됐다(신호 3종 픽스처).

**교훈은 게이트가 아니라 픽스처에 있다.** 게이트가 무력했던 것이 아니라 **픽스처가 값을
일치시켜 게이트에게 잴 것을 주지 않았다.** 단독 격추 0건은 그 증상을 재는 지표이고,
"통과했다"만 봐서는 절대 드러나지 않는다.

**남은 사각(문서화하고 수용).** Task 6 계약 리뷰가 Step 7b 패턴을 6형태에 직접 돌려
실측했다(앵커 `count == 1` 확인 · `sed` 범위 줄 수 병기). **6개 중 3개만 잡는다.**

| 형태 | Step 7b |
| --- | --- |
| 대조군(정상 코드) | 0건 — 오탐 없음 |
| `len(outcome.risks)` | **우회** |
| `risks = outcome.risks` 뒤 `len(risks)` | 잡음 |
| `len([r for r in outcome.risks if r.selected])` | **우회** |
| `len(outcome.selected)` | **우회** |
| `sum(1 for r ... if r.hard_fail)` | 잡음 |
| `Counter()` 루프 | 잡음 |

우회하는 **세 형태**는 두 게이트 어느 쪽도 잡지 못한다 — 오늘은 등가라 잡을 것이 없고,
프로퍼티의 정의가 바뀌면 Step 7b의 패턴에도 매치되지 않아 미래의 갈라짐도 놓친다.
(`len(outcome.selected)`는 이 실측 전까지 **어느 표에도 없던 형태**다 — 사각의 목록조차
측정하기 전에는 불완전했다.)

거기 서 있는 것은 `models.py:91-93`의 독스트링("두 값이 지금 같다는 것은 우연이지 보장이
아니다")뿐이다. 게이트를 CI에 넣는 것이 진짜 해법이고 이 브랜치 범위 밖이다 —
**브랜치 전체 리뷰가 판단한다.**
...
**파일 머리말 표 `cli.py:16`의 `| 70 | 미구현 | |` 행도 함께 고친다.** 그 표가 종료 코드의 단일 출처이고, 주석만 고치면 표와 갈라진다 — 다른 다섯 행은 전부 근거 열(`sysexits.h EX_NOINPUT` 등)을 갖는데 70만 비어 있는 것도 이때 채운다.
...
**같은 자리에서 하나 더 본다 — Task 4가 남긴 것이다.** `TriageOutcome` 생성자의 `ValueError`가 `_run_triage` 호출부의 `except ValueError`에 잡혀 **exit 2**로 나간다. 그 자리 주석은 "설정 오류라 2다"인데 **생성자 불변식 위반은 설정 오류가 아니라 내부 결함**이라 표식이 부정확하다.

지금 도달 경로는 없다 — `risks`와 `segments`가 같은 `kept`에서 나오므로 id 집합이 어긋날 수 없다(Task 4 구현자가 확인해 주석에 넷째 출처로 명시했다). 따라서 **동작을 바꾸는 것이 목적이 아니라, 종료 코드를 손보는 이 자리에서 그 사실을 다시 판정하는 것**이 목적이다. 도달 불가가 유지되면 주석만으로 충분하고, `--review-out` 배선이 그 전제를 깨뜨렸다면 분리해야 한다.
...
def test_화면_요약과_파일_수치가_일치한다(
    ...
    **화면이 실제로 다른 값을 찍는 갈라짐을 여기가 잡는다.** 화면과 파일이 한
    실행에 함께 존재하는 이 자리라야 값을 직접 대조할 수 있다.

    Task 4 Step 7b를 빠져나간 변이 둘(`len(outcome.risks)`·컴프리헨션)은 **여기서도
    잡히지 않는다** — 등가 변이라 잡을 결함이 없기 때문이다(Task 6 실측). 이 게이트가
    막는 것은 M5·M14·M15처럼 **값이 실제로 달라지는** 형태다.
...
기대: 새 테스트 **9개** 실패(파일이 만들어지지 않는다).

**10개가 아니다 — 이 예고는 틀렸었고 Task 6 실측이 정정했다.**
`test_dry_run은_파일을_쓰지_않는다`는 **부재를 단언**하므로(`assert not reports.exists() or ... == []`)
배선 전에도 자동으로 참이다. 즉 이 테스트는 **"구현 전 빨강"으로 자기 값을 증명할 수 없는
형태**다. 사후 변이 M7(dry-run 조기 반환 제거)에서 살해자에 포함되는 것은 확인했으나, 다른
dry-run 테스트 4개도 함께 죽으므로 **고립된 증거가 아니다.**
...
- [ ] **Step 1: 변이 13종을 하나씩 넣고 해당 테스트만 죽는지 확인한다**

**"12종"이 아니라 13종이다** — 아래 표는 `#2b`를 포함해 **13행**이다. 원래 머리말이 12라고 적어
구현자가 "어느 쪽이 진짜 개수인가"를 물었다. 표의 행 수가 진짜다.
...
- [ ] **Step 9: (구현자의 몫이 아니다) PR은 컨트롤러가 사용자에게 제시한다**

**이 절에는 실행할 명령이 없다. Task 8은 Step 8(커밋)에서 끝난다.**

원래 여기에 `git push -u origin ...` 코드 블록이 있었고 바로 아래 산문이 "푸시는 사용자가
명시적으로 요청할 때만 한다"고 말했다. **이 계획서의 다른 모든 코드 블록은 실행 지시였으므로**
구현자는 이것도 실행 지시로 읽는다 — 산문을 더 강하게 쓰는 것으로는 못 막고, 코드 블록을
없애야 막힌다.

푸시는 **위임할 수 없는 판단**이다. 사용자 지침이 "푸시는 사용자가 명시적으로 요청할 때만 실행 —
커밋과 푸시를 한 명령에 묶지 않는다. **긴급 수정이어도 예외 없음**"으로 못 박았고, 이것을
서브에이전트에게 "알아서 판단하라"로 넘기면 그 예외 없음이 예외를 갖는다.

컨트롤러가 브랜치 전체 리뷰를 마친 뒤 사용자에게 제시하고, 승인을 받으면 그때 푸시·PR을 만든다.
PR 본문에는 무엇을 · 근거 문서 · **게이트 수치**(개수를 그대로)를 담는다.
