...
**이 게이트는 AST가 아니라 텍스트를 본다.** 함수 범위 안 주석에 `sorted(`를 글자 그대로 쓰면 자기 게이트에 걸리고(Task 4 구현자의 초안이 실제로 그랬다), 반대로 다른 이름의 헬퍼나 리스트 컴프리헨션으로 계산을 빼돌리면 통과한다. **재는 것은 "이 네 패턴이 없다"이지 "계산이 없다"가 아니다.**
