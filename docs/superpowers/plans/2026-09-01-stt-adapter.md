...
| B6 | 사용자에게 나가는 문구 둘 — `load_input`의 "`--base-url`과 `--model`을 주거나", `_reject_non_subtitle`의 "STT는 v0.1에 없다(WBS WP9)" | **둘 다 오늘 참인 문장으로 바꿨다** (최종 픽스 라운드 1, F7-b) | 전자는 **CLI에 존재하지 않는 플래그**를 안내했다 — 그대로 쳐 본 사용자는 "알 수 없는 옵션"만 받는다. 후자는 WP9가 어댑터를 낸 지금 거짓이다 — 기능은 있고 CLI 배선만 없다 |

**B6은 시점이 다르다** — 태스크 7개가 끝난 뒤의 최종 픽스 라운드 1에서 잡힌 것이라
위 다섯처럼 구현 중에 드러난 것이 아니다. 아래 본문의 코드 블록은 옛 문구를 그대로
담고 있으므로 **복사하지 말 것.**

**표가 말하는 것은 구현 중에 드러난 다섯 중 셋이 실측으로 뒤집혔다는 것이다** — B1·B2·B3은 전부 코드를
    ...
    **이 함수를 부르는 것은 지금 테스트뿐이다.** CLI 배선이 FR-8.3(WP6)이라
    ...
    if media is not None:
        if provider is None:
            ...
            # **아래 안내 문구는 폐기됐다 (구현 중 바뀐 결정 B6).**
            # `--base-url`·`--model`은 STT용으로 CLI에 없다. 실제 코드는
            # "load_input에 provider를 넘기거나 자막 파일을 입력하라."다.
            raise IngestError(
                "video_input",
