...
가중 평균의 분모가 늘어나므로 **고정 0.5는 0.5 초과 점수를 항상 끌어내리고 0.5 미만을 항상
끌어올린다.** 실측(컨트롤러 독립 재현):

| 세그먼트 | `risk_score` |
| --- | --- |
| `spec.violation`=1.0 단독 | 1.0 |
| `spec.violation`=1.0 + `spec.overlap`=0.5 | **0.75** |

리뷰어의 합성 시나리오(N=200, 진짜 오류 20건, 겹침 30%)에서 예산 10% Recall이
**1.000 → 0.700**, 배수가 **10.0x → 7.0x**로 나왔다. 합성 조건이므로 실데이터 수치가 아니지만
**방향과 크기는 벤치 하네스가 어느 시점에서 baseline을 뜨느냐에 직결된다.**
...
| 3연쇄 겹침(a-b, b-c) | `b`·`c` 둘 다 flag. running-max 설계가 D-4 의도대로 동작 |
| 긴 세그먼트가 여러 개를 덮을 때 | `B`(100ms)·`C`(1000ms) 둘 다 정확한 겹침량으로 잡음 |
| `_RATIO_MIN_SOURCE_WIDTH`의 실제 컷오프 | **ko 4글자** — CJK는 세 counting mode 모두 1.0이다. `D-18` 커밋 메시지의 "1~2자"는 부정확했고, 주석에 실제 사각지대(3글자 이하)를 명시했다 |

**미검증으로 남은 것**: 사각지대(ko 3글자 이하)가 실제 오류를 얼마나 놓치는지의 정량,
`spec.overlap` 점수 0.5 고정이 겹침 크기 정보를 버리는 것의 타당성, `Span.side` 기본값
`"target"`이 향후 `glossary.miss`·`struct.number_missing` 구현에서 틀린 쪽을 칠할 위험.
**셋 다 벤치 하네스 이후로 미룬다.**
