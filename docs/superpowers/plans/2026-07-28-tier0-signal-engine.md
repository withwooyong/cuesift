...
| # | 조건 | 확인 방법 |
| --- | --- | --- |
| 1 | Tier 0 신호가 모두 등록된다 | `python -c "from cuesift.signals import registry; print(len(registry()))"` → `9` (최종 리뷰에서 `spec.overlap` 추가, 아래 §D-14) |
...
> 실행 완료 후 추가 (2026-07-28). 브랜치 `feat/tier0-signal-engine`, 커밋 26개, 테스트 172건.

**이 절은 위 계획과 실제 구현의 차이를 기록한다.** 위 본문의 코드 블록은 **고치지 않았다** — 최종 코드로 갈아 끼우면 계획이 "구현과 같은 것을 두 번 적은 문서"가 되고, 실제 코드가 항상 더 정확하니 존재 이유가 사라진다. 남길 가치가 있는 것은 **차이 그 자체**다.

**따라서 위 본문의 코드 블록은 신뢰하지 말고 `src/`를 보라.** 이 절이 왜 달라졌는지를 설명한다.
...
| 수정 라운드 | 9회 |
| 리뷰가 걸러낸 실질 결함 | **17건** |
| 그중 계획(브리프) 자체의 결함 | **15건** |
| 최종 브랜치 리뷰 판정 | 병합 가능 (Critical 0건) |

**구현자들은 브리프를 정확히 따랐다.** 오류의 발원지는 구현이 아니라 설계였다.

### 결함의 공통 형태

15건이 거의 전부 같은 모양이었다 — **"흔한 입력에서는 맞는" 코드**다.

| 결함 | 맞는 경우 | 틀리는 경계 |
| --- | --- | --- |
| `_WIDE`에 `A` 포함 | ASCII만 있는 텍스트 | 악센트 라틴 (`é` vs `É`) |
| 인접 쌍 겹침 검사 | 자막이 서로 포함하지 않을 때 | 긴 자막이 여러 개를 덮을 때 |
| 부분 문자열 용어 매칭 | 긴 용어 | 짧은 약어 (`AI` ⊂ `rain`) |
| 전체 최빈 토큰 반복 검사 | 짧은 문장 | 관사가 3회 나오는 평범한 영어 |
| MAD 척도 | 정상군이 흩어져 있을 때 | 조밀하게 뭉칠 때 |
| `weight < 0` 검사 | 실수 가중치 | `nan` (비교가 `False`) |

그리고 **계획에 내가 쓴 테스트가 정확히 그 "흔한 경우"만 담았기 때문에** 계획 단계의 자체 검토로는 걸러지지 않았다. 테스트를 쓴 사람과 코드를 쓴 사람이 같으면 같은 맹점을 공유한다.

**효과가 있었던 것은 리뷰어에게 구체적 위험을 지목한 것이다.** "3개 이상 연쇄로 겹칠 때를 확인하라", "짧은 용어의 부분 문자열 오탐을 확인하라" 같은 지시가 그대로 적중했다. 반대로 "잘 리뷰해 달라"고만 하면 diff를 읽고 "브리프대로 구현됨"으로 승인된다.

### D-1 ~ D-15 · 계획 결함과 수정

#### D-1 `latin_half`가 악센트 라틴을 전각으로 셌다 (Task 2)

계획은 `_WIDE = frozenset({"W", "F", "A"})`를 지시하며 "CJK 문맥에서 `±`·`°`가 전각으로 렌더링된다"를 근거로 달았다. 근거 자체는 맞지만 **`_WIDE`가 실제로 쓰이는 유일한 곳이 `latin_half`** 다 — `grapheme`과 `fullwidth`는 `len(chars)`만 쓴다. 즉 CJK 문맥을 위한 규칙이 "라틴은 반각" 모드에만 적용되고 있었다.

결과가 특히 나빴다: `é`(U+00E9)는 `A`라서 1.0, `É`(U+00C9)는 `N`이라서 0.5 — **같은 글자의 대소문자가 다른 폭**이다. 유니코드의 Ambiguous 등급은 역사적 인코딩(CJK 레거시 코드페이지 수록 여부)을 반영한 것이라 라틴 악센트 문자를 자의적으로 가른다.

**결정**: `_WIDE = frozenset({"W", "F"})`. `±`·`°`·`×`가 0.5로 과소계산되지만 일관성을 택했다.

#### D-2 `fullwidth`와 `grapheme` 분기를 합치지 않는다 (Task 2, 사전 판정)

계획이 "값이 같아도 분기를 유지하라"고 지시했고, 실행 전 스캔에서 리뷰 규칙상 중복으로 지적될 것이 예상돼 사람에게 확인받았다. **유지 결정.** 두 모드는 정의가 다르고 값만 우연히 같다 — `grapheme`이 나중에 진짜 자소 클러스터 분할로 바뀌어도 `fullwidth`는 "보이는 문자 = 전각 1"이라는 자체 정의를 유지해야 한다.

최종 리뷰가 이 결정을 지지했다: 실제로 `fullwidth`(ted-ja)와 `grapheme`(ted-en)이 서로 다른 프로파일에 쓰이고 있어 이름이 둘일 이유가 코드로 증명된다.

#### D-3 TED 프로파일 값을 조사로 확정했다 (Task 3)

계획은 "TED가 ko·ja를 명시하지 않으면 라틴 기준 환산"이라는 대비책과 함께 CPS 11을 적었다. **조사 결과 TED는 두 언어에 별도 값(21자/줄, 10 CPS)을 정해 두고 있었다.** 환산치와 우연히 가까웠지만 근거가 완전히 다르다 — 벤치마크가 TED 데이터를 TED 기준으로 재려는 것이므로 이 차이가 측정의 정당성 자체다.

**출처 URL이 죽었다.** `translations.ted.com`이 DNS 해석에 실패한다(2026-07-28 확인). TED가 Open Translation Project 위키를 내린 것으로 보인다. 값은 검색 색인 스냅샷으로만 확인했고, 두 언어 포털이 독립적으로 같은 수치를 말하는 것이 상호 검증 역할을 한다. **이 사실을 YAML 주석과 테스트에 남겼다** — 다시 확인할 수 없는 수치는 테스트가 유일한 보존 수단이다.

TED 프로파일 3종의 노출시간은 833/7000으로 통일했다. TED가 지속시간을 명시하지 않으므로 Netflix ja의 500ms를 가져올 근거가 없고, 근거 없는 비대칭은 벤치마크에서 ja만 다른 기준으로 재게 만든다.

#### D-4 겹침 탐지가 인접 쌍만 비교했다 (Task 4)

계획의 `zip(ordered, ordered[1:])`는 "정렬했으니 인접한 것만 보면 된다"는 직관에서 나왔는데, 이 직관은 **구간이 서로를 포함하지 않을 때만** 성립한다. `A(0~10000)`가 `C(5000~6000)`를 덮어도 사이의 `B(100~200)`와 `C`가 안 겹치면 `C`가 검사에서 통째로 빠졌다.

**두 번째 결함이 함께 드러났다**: `measured`가 `prev.end - curr.start`인데, 이는 뒤 세그먼트가 앞의 끝을 넘어설 때만 맞는 식이다. `B`는 실제 겹침이 100ms인데 9900ms로 보고됐다. **이 값은 `review.json`에 실려 검수자가 보는 숫자다.**

**결정**: "지금까지 본 최대 `end_ms`" 스윕으로 교체하고 `measured = min(run_end, seg.end_ms) - seg.start_ms`로 보정. (본문 Task 4 절은 이미 갱신돼 있다.)

#### D-5 용어집 부분 문자열 매칭이 위반을 놓쳤다 (Task 5)

`t.lower() in lowered`가 단어 중간에 걸렸다. 용어집의 대응어가 `AI`일 때 번역문 `"It might rain today"`가 `rain` 안의 `ai` 때문에 "위반 없음"이 됐다. **계획의 예시 용어집이 `AI`를 쓰고 있어 계획 자신의 데이터로 재현된다.**

**리뷰어의 수정안(`\b` 단어 경계)을 기각했다.** `\b`는 `\w`와 `\W` 사이에서만 성립하는데 **CJK 문자는 파이썬 정규식에서 전부 `\w`** 라, `\b気候変動\b`는 「これは気候変動です」에서 매칭되지 않는다. 이 프로젝트의 언어쌍이 ko→en/ja이므로 그 수정은 한국어·일본어 용어를 전부 미매칭으로 만들어 **모든 세그먼트에서 용어 위반이 발화**하게 했을 것이다 — 원래 버그보다 훨씬 파괴적이다.

**결정**: 룩어라운드 `(?<![a-zA-Z0-9])term(?![a-zA-Z0-9])`. 라틴 문자·숫자만 경계로 보므로 CJK 이웃에 영향받지 않는다. 8개 케이스 실측 확인.

함께 고친 것: `targets`가 리스트가 아니면 거부(YAML 대괄호 누락 시 글자 단위로 쪼개져 검사가 영원히 통과), 원문 쪽에도 대소문자 무시 적용, 항목 단위 `ValueError`.

#### D-6 반복 붕괴가 정상 영어 문장에서 발화했다 (Task 7)

`Counter(tokens).most_common(1)`은 **연속 반복이 아니라 텍스트 전체의 최빈 토큰**을 센다. `"the cat sat on the mat with the dog"`가 발화했다 — 관사가 3회 나오기 때문이다.

**`hard_fail=True`라 치명적이었다.** FR-6.2에 따라 hard fail은 검수 예산을 우회하므로, 보통 길이의 영어 문장 상당수가 예산과 무관하게 큐로 직행한다. 그러면 §9.1의 실제 검수 비율이 폭증하고 배수가 1에 수렴한다 — **이 버그 하나로 "AI 래퍼가 아니다"를 증명할 숫자가 사라진다.**

**결정**: 연속 n-gram(단위 최대 4어절, 최소 3회)만 본다. FR-3.3이 "어절·**구**"라고 했으므로 구 단위 반복(`"I don't know"` ×3)도 잡는다.

#### D-7 그 수정이 O(n²)를 낳았다 (Task 7, 라운드 2)

D-6의 슬라이딩 윈도우가 이차 복잡도였고, **하필 이 신호가 잡으려는 입력이 최악 케이스**였다 — 디코딩 루프에 빠진 LLM은 같은 토큰을 수천 번 뱉는다. 실측 5000토큰 2479ms. 탐지기가 탐지 대상 앞에서 멈춘다.

**결정**: `_DEGENERATION_MAX_TOKENS = 200` 상한. 연속 반복은 앞쪽에서 이미 드러나므로 탐지력 손실이 없고, 자막 한 줄이 200어절을 넘는 것 자체가 비정상이다. 수정 후 3.8ms 상수 시간(토큰 20000에서도 동일).

#### D-8 태그 비교가 표기 차이를 손실로 오판했다 (Task 7)

태그 문자열을 정규화 없이 비교해 따옴표 스타일(`color='red'` vs `color="red"`), 속성 내부 공백, `<br>` vs `<br/>`에서 전부 hard fail이 났다. 자막 파이프라인은 편집기·파서 라운드트립에서 태그를 재직렬화하므로 흔한 상황이다.

**결정**: 태그 **이름만** 비교한다(속성 무시, 닫는 태그는 `/` 접두어). 속성 값 변화(red→blue)를 못 잡는 것은 **의도된 한계**다 — hard fail 신호에서는 미탐이 오탐보다 낫다.

#### D-9 숫자 검사가 천 단위 콤마에서 깨졌다 (Task 7)

`\d+`가 `1,000`을 `['1','000']`으로, `3.14`를 `['3','14']`로 쪼갰다. 원문은 콤마를 쓰고 번역문은 안 쓰는 일이 흔해(영어 자막 관행) 온전히 보존된 숫자가 누락으로 잡혔다.

**결정**: `\d[\d,]*(?:\.\d+)?`로 뽑고 콤마를 제거해 정규화.

#### D-10 한 자리 수 누락의 hard fail을 해제했다 (Task 7)

원문 `"3시"` → 번역 `"See you at three"`가 발화했다. **영어 자막에서 한 자리 수를 단어로 적는 것은 표준 관행**(Netflix 스타일가이드)이라 정상 번역이 예산을 우회해 큐에 쌓인다.

**결정**: 누락된 숫자가 전부 한 자리면 `hard_fail=False`, `score=0.5`. 두 자리 이상(연도·금액·시각)은 단어로 적는 일이 거의 없으므로 hard fail 유지. **언어별 숫자-단어 표를 만들지 않았다** — 이 프로젝트가 경계해 온 "언어별 예외 목록"을 다시 만들지 않기 위해서다.

#### D-11 미번역 판정에 절대 문자 수 하한을 더했다 (Task 7)

비율 분모가 번역문 전체 길이라 짧은 대사에서 한 글자만 섞여도 임계를 넘었다 — `"Hi 가"`가 1/4 = 25%다. 자막은 감탄사·짧은 응답이 매우 흔하다.

**결정**: 비율 15%에 더해 `_UNTRANSLATED_MIN_CHARS = 2`를 함께 요구.

#### D-12 길이비 척도가 MAD 미소 시 붕괴했다 (Task 8) — 그리고 첫 수정이 틀렸다

계획은 `scale == 0`일 때**만** 평균절대편차로 폴백했다. MAD가 **0은 아니지만 아주 작은** 경우(번역 스타일이 일관된 정상 트랙)에 척도가 무너져 **중앙값 대비 0.4% 편차가 z=4.72**가 됐다.

**첫 수정(`max(MAD, 평균절대편차)`)이 틀렸다.** 평균절대편차는 로버스트하지 않아 이상치 자체가 척도를 부풀린다. 500 세그먼트 시뮬레이션:

| 주입률 | `max()` 재현율 | MAD 재현율 |
| --- | --- | --- |
| 5% | 100% | 100% |
| **10%** (§9.2 벤치마크 기본값) | **50%** | 100% |
| 30% | **0%** | 100% |

**원인은 검증 방식이었다** — 손으로 만든 4개 케이스만 봤고, 결정적 조건(주입률 10%)이 스펙 §9.2에 이미 적혀 있는데도 시뮬레이션하지 않았다. 손으로 고른 예제는 작성자의 가정을 공유한다.

**최종 결정**: MAD를 주 척도로 되돌리고(0일 때만 평균절대편차 폴백) **"통계적으로 극단"과 "실질적으로 다름" 두 조건을 모두 요구**한다 — `_RATIO_MIN_RELATIVE_DEVIATION = 0.25`. 세 시나리오(주입률 5~30%, 조밀 정상군, 균일 정상군) 전부에서 재현율 100%·오탐 0.

**회귀 테스트도 한 번 더 고쳤다.** 첫 회귀 테스트가 `max()` 코드에서도 통과했다 — 이상치가 **동종**이라 척도가 무너지지 않았기 때문이다. 평균절대편차 척도는 이상치가 **이종**일 때 무너진다(먼 것이 척도를 부풀려 가까운 것을 정상 범위로 밀어 넣는다). 실제 주입 오류가 정확히 이종이다(미번역 ~1.0, 반복 붕괴 ~10.0). **회귀 코드에서 통과하는 회귀 테스트는 회귀 테스트가 아니다.**

#### D-13 신호 간 점수 스케일을 통일했다 (Task 8)

`spec.violation`은 위반 건수에 비례해 0.33~1.0을 냈는데 `glossary.miss`는 몇 건이든 항상 1.0이었다. 균등 가중 평균에서 이는 **용어 위반에 3배 가중**을 준 것과 같다. 계획 §6.3이 "가중치는 튜닝하지 않는다"고 못 박고 `DEFAULT_WEIGHTS`를 전부 1.0으로 뒀는데, **점수 스케일을 통해 가중치가 이미 들어가 있었다.**

**결정**: 공용 `_violation_score(count)` — 1건=0.5, 2건=0.75, 3건 이상=1.0. 하한 0.5는 "한 건만 있어도 이미 유의미하다"를 표현한다.

각 신호를 따로 보면 둘 다 합리적이다("위반이 많을수록 위험하다"도, "용어 오역은 그 자체로 위험하다"도). **비교했을 때만 어긋남이 보인다.**

#### D-14 겹침 검사가 어떤 신호에도 연결되지 않았다 (최종 리뷰)

`check_overlaps`는 Task 4에서 구현되고 테스트 5건이 붙고 결함 2건까지 고쳐졌으나 **호출자가 없었다.** `SpecViolationSignal`은 `check_text`만 부르고, `check_text`는 세그먼트 하나만 보므로 겹침을 판정할 수 없다. 즉 **FR-5.1의 중첩 금지가 위험도에 도달하지 않았다.**

개별 태스크 리뷰가 구조상 볼 수 없는 결함이다 — Task 4 리뷰는 자기 diff만 봤고 그 안에서는 완벽했다. Task 8은 자기 diff에 없는 함수를 부를 이유가 없었다. **아무도 "이게 실제로 호출되는가"를 묻지 않았다.**

**결정**: `OverlapSignal` 배치 수집기 추가. **hard fail이 아니다** — 겹침은 타이밍 결함이지 번역 결함이 아니고, hard fail을 늘리면 예산 우회가 커진다. 이로써 **Tier 0 신호가 9종이 됐다** (계획 본문의 "8종"은 낡았다).

#### D-15 `review_ratio`가 오용 API였다 (Task 10)

`select_by_budget`이 선별된 것만 반환하므로 `review_ratio(select_by_budget(...))`라는 가장 자연스러운 호출이 **항상 1.0**을 냈다. 예외도 경고도 없이 **README 최상단 배수의 분모가 1이 된다.**

**계획의 테스트가 결함의 증거였다.** `test_review_ratio_reports_what_was_actually_spent`가 "선택 사본 + 원본에서 안 뽑힌 것들"을 손으로 재조립하고 있었다 — 올바르게 쓰려면 그런 재조립이 필요하다는 것 자체가 API가 틀렸다는 신호다.

그리고 `SegmentRisk`에 `selected` 필드가 있는데 선별된 것만 반환하면 그 필드는 항상 참이라 잉여다. **모델이 이미 답을 알려주고 있었다.**

**결정**: `select_by_*`가 **전체 목록**을 반환하고 선별된 것에만 `selected=True`를 붙인다. 검수 큐가 필요하면 `[r for r in result if r.selected]`로 거른다.

함께 고친 것: hard fail 구간도 전체 정렬(입력 순서 의존 제거), `_copy`가 `signals`·`reasons`까지 복사(비선별 항목 포함), `math.isnan` 명시 검사.

### 그 밖의 수정

| # | 항목 | 태스크 |
| --- | --- | --- |
| D-16 | `nan`·`inf` 가중치가 최대 위험을 최소 위험으로 뒤집었다 | 9 |
| D-17 | 가중치 총합이 0이면 조용히 안전 판정됐다 | 9 |
| D-18 | `length.ratio`가 짧은 원문에서 계통 오탐(400건 중 33건) | 최종 리뷰 |
| D-19 | `select_by_threshold`에 인자 검증이 없었다 | 최종 리뷰 |
| D-20 | `Span`에 원문/번역문 판별자가 없었다 | 최종 리뷰 |
| D-21 | `SegmentRisk`에 범위 검증이 없었다 | 최종 리뷰 |

**D-16이 특히 교훈적이다.** 파이썬의 두 가지 조용한 동작이 겹쳤다 — `nan < 0`은 `False`라 음수 검사를 통과하고, `max(0.0, nan)`은 `0.0`을 반환해 NaN이 전파되지 않고 **사라진다.** 그리고 YAML이 `.nan`·`.inf`를 파싱하므로 FR-8.4 설정 파일 오타 하나로 도달한다. **클램프는 값을 범위 안에 넣지만 값이 의미 있는지는 보장하지 않는다** — 오히려 범위 밖이라면 드러났을 오류를 그럴듯한 값으로 바꿔 숨긴다.

**D-21에서 리뷰어 제안의 절반을 기각했다.** `hard_fail=True`면 `risk_score`가 1.0이어야 한다는 정합성 검사를 제안받았으나, 이는 `triage`의 의미 보장을 `risk`의 구현 선택에 묶는다. FR-6.2는 "hard fail이 가중합을 **우회**한다"는 의미 계약이지 "항상 1.0"이 아니다. 범위 검사만 넣었다.

### 재리뷰 (병합 전, 2026-07-28)

최종 수정 5건(`bfa0f72`..`cace3d4`)이 재리뷰를 받지 않은 채 남아 있어, 위험 축을 둘로 나눠
리뷰어를 붙이고 컨트롤러가 핵심 주장을 독립 재현했다. **Critical 0건.**

| # | 위치 | 심각도 | 내용 | 이번 5건의 회귀인가 |
| --- | --- | --- | --- | --- |
| D-22 | `risk/fuse.py` | Major | 가중치 **합계** 오버플로가 D-16의 사각지대였다 | 아니오 (Task 9부터) |
| — | `risk/fuse.py:29` | Major | `spec.overlap` 연결로 융합 희석 노출면 확대 | 아니오 (기존 8종에도 동일) |
| — | `triage/policy.py:23-38` | Minor | `_copy()`가 `Signal` 객체·`detail` 딕셔너리를 공유 | 신규(잠재) |
| — | `triage/policy.py:83` | Minor | hard fail이 quota를 소진해 상위 비-hard를 밀어냄 | 신규(문서화 누락) |

#### D-22 가중치 **합계** 오버플로 — D-16이 개별 값만 막고 있었다

D-16이 `math.isfinite(weight)`로 `nan`·`inf`를 막았으나 **항목 하나씩만** 검사했다.
`1e308` 두 개는 개별로는 유한해 전부 통과하고, 합이 `inf`가 되면
`weighted / total_weight = inf / inf = nan` → `min(1.0, max(0.0, nan)) = 0.0`이다.

**D-16 주석이 "막았다"고 선언한 실패 모드가 합계 경로로 그대로 재현됐다** — 최대 위험(1.0)의
신호가 예외도 로그도 없이 최소 위험이 된다. `검사하지 않고 통과하는 게이트는 없는 게이트보다
나쁘다`에 정면으로 걸리는 형태다.

`cace3d4`가 추가한 `SegmentRisk.__post_init__` 범위 검증은 **이 경로에서 발동하지 않는다.**
clamp가 먼저 NaN을 0.0으로 바꿔 버리므로 검증은 NaN을 본 적조차 없다.

**결정**: `total_weight`에 `math.isfinite` 검사 추가. 회귀 테스트 2건(오버플로 거부 + `1e307`
합계는 통과)을 붙였고, **수정 전 코드에서 `DID NOT RAISE`로 실패하는 것을 확인한 뒤** 고쳤다.

#### `spec.overlap` 연결이 벤치마크 baseline을 이동시켰다 (기록 전용)

D-14가 `OverlapSignal`을 연결하면서 신호가 9종이 됐다. **융합 규칙도 가중치도 바꾸지 않았으므로
"베이스라인 확정 전 융합 변경 금지" 결정과 정합적이다.** 그러나 이 신호는 기존 8종과 성질이 다르다.

| 항목 | 기존 8종 | `spec.overlap` |
| --- | --- | --- |
| 점수 | 위반 건수에 따라 0.5~1.0 변동 | **항상 0.5 고정** |
| 판정 근거 | 번역 품질 | 타이밍 (번역과 무관) |
| 발생 패턴 | 세그먼트 개별 | 계통적 — `end_ms` 오타 하나가 뒤따르는 세그먼트를 대량 플래그 |

가중 평균의 분모가 늘어나므로 고정 0.5는 기존 점수를 이동시킨다. **v0.1에서는 방향이 한쪽뿐이다.**

Tier 0 9종의 최소 점수를 전수 확인한 결과 **0.5 미만을 내는 신호가 없다**
(`length.ratio`조차 게이트 `z > 3.5` 때문에 하한이 0.5000014다). 따라서 "0.5 미만을
끌어올린다"는 v0.1에서 도달 불가이고, **soft 신호를 가진 세그먼트의 위험도는 내려가기만 한다.**
hard fail은 `fuse.py`가 가중합을 우회하므로 면역이라, 피해는 **예산을 두고 경쟁하는
soft 신호 4종에 정확히 집중된다.**

| 확인 | 값 |
| --- | --- |
| `spec.violation`=1.0 단독 → `+spec.overlap` | 1.0 → **0.75** (컨트롤러 독립 재현) |
| soft 신호 전수 159조합 | 상승 **0건** / 하락 **152건** / 동일 7건, 최대 하락폭 **−0.25** |
| hard fail + `spec.overlap` | 1.0 유지 (희석 없음) |

**그리고 반대 방향의 오염이 더 크다.** `check_overlaps`가 누적 최대 `end_ms`와 비교하므로
(D-4의 의도된 설계) **단일 타임코드 오타 하나가 트랙 대부분을 flag한다** — 컨트롤러 실측:
...
즉 번역상 결백한 세그먼트 수십~수백 건이 0.0 → 0.5로 올라가고, 동시에 진짜 오역이 있는
세그먼트가 최대 0.25 내려간다. **Recall@Budget이 의존하는 순위가 양쪽에서 동시에 무너진다.**

**Recall 손실의 크기는 두 리뷰어가 엇갈린다 — 어느 한쪽만 읽으면 안 된다.**

| 측정 | 조건 | 예산 10% Recall |
| --- | --- | --- |
| 리뷰어 A | N=200, 오류 20건, 겹침 30%, 위험도 동점대 밀집 | 1.000 → **0.700** (배수 10.0x → 7.0x) |
| 리뷰어 B | 몬테카를로 200건 × 30시드, 오역 3.0~6.0x 확장 | 1.000 → **1.000** (90시행 하락 0건) |

**메커니즘은 전수로 확정됐지만 발현 조건이 좁다.** 무는 조건은 위험도 동점대가 붐빌 때이고,
실제 트랙에서 `spec.violation`(CPS·줄길이)이 광범위 발화하면 그 조건이 성립한다.
**둘 다 합성 조건이므로 실데이터 수치가 아니다.**

**결정**: 지금 고치지 않는다. Known Issue의 "첫 측정 전에 규칙을 바꾸면 베이스라인이 없어
개선을 입증할 수 없다"가 그대로 적용된다. **벤치 하네스의 첫 측정에서 `spec.overlap` on/off를
ablation 항목으로 넣어 A/B로 판정할 것** — `collect_all(segments, ctx, enabled=[...])`이 이미
그것을 지원한다.

**계획 문서가 앞서 기록한 "20시드 평균 예산 10%에서 5.2%p 손실"은 이 신호가 연결되기 전 값이다.**
`spec.overlap`을 입력으로 갖지 않은 상태의 측정이므로, 벤치 A/B 목록에 이 항목을 빠뜨리면
다음 세션이 낡은 기준선을 유효한 것으로 읽는다.

#### 기각된 의심 — 확인 결과 문제가 아니었던 것

브리프가 지목한 위험 중 실측으로 기각된 것들이다. **지목이 적중을 보장하지 않는다는 기록이기도 하다.**

| 의심 | 확인 결과 |
| --- | --- |
| `check_overlaps`가 입력 정렬에 의존하는가 | **아니오.** 내부에서 `sorted()`. 역순 입력도 동일 결과 |
| 3연쇄 겹침(a-b, b-c)에서 중간 고리가 빠지는가 | **아니오.** `b`·`c` 둘 다 flag. running-max 설계가 D-4 의도대로 동작 (회귀 테스트 추가) |
| 긴 세그먼트가 여러 개를 덮을 때 | `B`(100ms)·`C`(1000ms) 둘 다 정확한 겹침량으로 잡음 |
| `_RATIO_MIN_SOURCE_WIDTH`가 mode에 따라 다른 뜻이 되는가 | **아니오.** CJK는 세 counting mode 모두 1.0이라 ko 컷오프는 어디서나 **4글자**. 다만 `D-18` 커밋 메시지의 "1~2자"가 부정확했고, 실제 사각지대(3글자 이하)를 주석에 명시했다 |
| 점수 0.5 고정이 "몰래 가중" 규율 위반인가 | **아니오.** `_violation_score(1)`과 정확히 같고 다른 신호 하한과 스케일이 맞다. 문제는 스케일이 아니라 **분모 증가**다(위 참조) |

#### D-23 `Span.side`에 런타임 검증이 없었다 (재리뷰)

`cace3d4`가 `SegmentRisk`에 "형제 세 모델과 같은 방어"를 근거로 범위 검증을 추가하면서,
**같은 커밋이 새로 넣은 `Span.side`는 무방비로 뒀다.** `Literal`은 타입 힌트일 뿐이라
`"both"`·`"TARGET"`·`""`·`None`·`0`이 전부 그대로 생성된다(실측) — 바로 두 줄 아래
`__post_init__`이 `start`/`end`는 이미 막고 있는데도 그렇다.

`side`가 틀리면 FR-7.3 리포트가 칠할 쪽을 잃는데, 그 사실은 신호를 만든 코드에서 멀리 떨어진
리포트 생성 시점에야 드러난다. **결정**: `__post_init__`에 한 줄 검증 추가(테스트 5건).
필수 인자로 바꾸는 것은 기각했다 — 기본값 `"target"`은 7종 텍스트 신호 중 5종이 맞으므로
합리적이고, `side` 오류는 하이라이트 위치만 틀릴 뿐 탐지에 영향이 없어
"미탐이 오탐보다 낫다"가 이 필드에는 적용되지 않는다.

#### D-24 `length.ratio` 하한이 만든 완전 사각지대 (재리뷰, 미수정)

D-18의 `_RATIO_MIN_SOURCE_WIDTH`는 오탐 33건을 없앴지만, 폭 4 미만 원문에 **길이비 판정이
아예 없다.** 대부분의 환각 확장은 `spec.violation`(CPS 초과)이 대신 잡지만
**표시시간이 길면 CPS도 통과해 9종 어느 것도 발화하지 않는다** — 리뷰어 실측:

```text
'맞아요'(3자) -> 'That is exactly correct, yes.'  (29자, 7000ms, CPS 4.1, 줄길이 29 < 42)
  하한 있음: 신호 없음          risk_score = 0.0
  하한 제거: ['length.ratio']   risk_score = 1.0     길이비 9.67 (정상군 중앙값의 4배)
```

**고치지 않는다.** 하한을 낮추면 D-18이 없앤 계통 오탐 33건이 돌아온다. `length.ratio`는
hard fail이 아니라 미탐이 검수 예산을 낭비하지 않으므로 §9.1 지표를 파괴하지도 않는다.
**벤치 하네스가 이 상수를 스윕할 때 사각지대의 크기를 함께 재는 것이 순서다.**

#### 재리뷰에서 확인만 하고 넘어간 것

| 항목 | 확인 결과 |
| --- | --- |
| `overlap_ms`가 `float`이다 (`500.0`) | 다른 신호 `detail`은 `int`/`str`. §8.4 `review.json`에 `500.0`으로 나간다. 기능 영향 없어 두었다 |
| 같은 id가 여러 개일 때 겹침이 접힌다 | `Segment`가 id 유일성을 강제하지 않는다. **로더가 없어 현재는 이론적** — `ingest` 구현 시 재확인할 것 |
| `spec.overlap`이 겹침 **크기**를 버린다 | 10ms와 3000ms가 같은 0.5다. 점수 스케일 통일(D-13)의 결과이며 "몰래 가중" 위반은 아니다 |

**미검증으로 남은 것**: `derived.py`의 `_RATIO_MIN_SOURCE_WIDTH` 근거 수치
("400건 트랙 33건→0건 / 주입 40건 재현율 100%")가 **저장소에서 재현 불가능하다** —
오류 주입기가 없어 일회성 스크립트로 측정했다. 그 사실을 주석에 명시했다.
**재현할 수 없는 근거는 근거가 아니라 기억이다.**

### 유지한 결정 (리뷰 지적에도 불구하고)

| 항목 | 근거 |
| --- | --- |
| `registry()`가 내부 dict 참조를 반환 | 테스트의 저장·복원이 이 함수의 존재 이유다. 사본을 반환하면 그 용도가 깨진다 |
| 반복 검사 200어절 상한으로 인한 미탐 | 자막 한 줄 250어절은 비현실적이고, 상한이 없으면 O(n²)가 살아난다 |
| `_TAG`가 `x<y>z`를 가짜 태그로 잡음 | 발화 조건이 좁고(`<` 직후 라틴 문자 + `>`), 비교가 대칭이라 원문의 가짜 태그가 번역문에 보존되면 발화하지 않는다. 고치려면 한 글자 태그명을 막아야 하는데 그러면 가장 흔한 `<i>`·`<b>`가 빠진다 |
| `length.ratio`가 원문·번역문에 같은 `char_counting` 사용 | v0.1 범위(ko→en/ja)에서 타깃 프로파일이 `grapheme`·`fullwidth`인데 **둘 다 한글을 1.0/자로 센다.** X→ko에서만 문제가 되고 범위 밖이다 |
| `detail["deviation"]`이 파생 중복값 | `review.json` 소비자가 재계산하지 않아도 되는 편의값. 비용이 없다 |

### 다음 계획으로 넘기는 것

#### 벤치마크로 판정할 항목

**가중 평균 융합의 구조적 문제.** 문제를 하나 더 찾으면 위험도가 **내려간다** — 규격 위반 3건(1.0)에 용어 위반 1건(0.5)을 더하면 0.75가 된다. 예산이 빠듯한 트리아지에서 문제가 가장 많은 세그먼트가 탈락한다. 합성 데이터 20시드 평균으로 예산 10%에서 재현율 5.2%p 손실이 측정됐고, noisy-or 계열이 구조적으로 맞다는 분석이 나왔다.

**지금 바꾸지 않는다.** 수치가 합성 데이터에서 나왔고, 첫 측정 전에 규칙을 바꾸면 베이스라인이 없어 개선을 입증할 수 없다. **벤치 하네스의 첫 측정 항목으로 A/B 판정할 것.**

함께 스윕할 상수: `_RATIO_MIN_RELATIVE_DEVIATION`(0.25) · `_RATIO_MIN_SOURCE_WIDTH`(4.0) · `_RATIO_MIN_SAMPLES`(8).
...
| 2 | **원문·번역 정렬(pairing)이 없다.** `Segment`는 둘을 한 객체에 갖는데 실제로는 두 파일에서 온다. 개수가 다를 때의 방침도 필요하다 |
| 3 | **정답 집합과 오류 주입기가 없다.** 벤치의 핵심이다 |
| 4 | **`SegmentRisk`에 세그먼트 본문이 없다.** `segment_id`만 갖는다. `review.json`을 쓰려면 리포트 계층이 `dict[str, Segment]`로 조인해야 한다 — 결함이 아니라 §7.2 모듈 경계 유지의 결과이며, 다음 계획이 이 조인을 명시적으로 들고 있어야 한다 |

#### `spans`가 전부 비어 있다

신호 9종 중 **어느 것도 `Span`을 채우지 않는다.** 현 상태로 FR-7.3의 리포트 하이라이트는 성립하지 않는다.

다만 **대부분 싸게 채울 수 있다** — 데이터가 이미 계산돼 있고 버려질 뿐이다. `struct.untranslated`는 `findall` 대신 `finditer`, `glossary.miss`는 `re.search`의 `match.span()`, `struct.number_missing`·`struct.tag_lost`도 이미 match 객체를 만든다. `spec.violation`은 `line_index`가 있으니 줄 오프셋만 누적하면 된다.

**D-20의 `Span.side`가 이것을 위한 준비다.** `glossary.miss`·`struct.number_missing`은 **원문**을 가리키고(누락이므로 번역문에 없다) 나머지는 **번역문**을 가리킨다. 판별자가 없으면 리포트가 어느 쪽을 칠할지 알 수 없다.

### 프로세스 기록

이 계획을 실행하며 컨트롤러(나)가 저지른 실패다. 다음 실행에서 되풀이하지 않기 위해 남긴다.

| # | 실패 | 결과 |
| --- | --- | --- |
| 1 | 구현자가 도는 중에 리포 파일을 편집했다 | 계획 문서 변경이 Task 4 커밋에 휩쓸렸다. 구현자 종료 후 커밋을 분리했다. **서브에이전트가 도는 동안 작업트리는 내 것이 아니다** |
| 2 | 수정안을 손으로 만든 4개 케이스로만 검증했다 | 벤치마크 조건(주입률 10%)을 놓쳐 재현율이 절반으로 떨어지는 수정을 지시했다(D-12) |
| 3 | 회귀 테스트가 회귀를 잡는지 확인하지 않았다 | 버그 코드에서도 통과하는 테스트를 만들었다. 구현자가 "수정 전 실패 확인" 절차를 밟다가 발견했다 |
| 4 | 테스트 개수를 두 번 잘못 셌다 | 구현자가 둘 다 잡았다. **기준값은 계획이 아니라 직전에 실제로 본 값이어야 한다** |
| 5 | 수정 지시가 기존 테스트와 논리적으로 충돌했다 | 한 자리 수 hard fail 해제(D-10)가 한 자리 수를 쓰는 기존 테스트와 양립 불가였다. 구현자가 픽스처를 두 자리로 바꿔 취지를 보존했다 |

**리뷰어에게 위험을 지목하는 것과 검증을 보장받는 것은 다르다.** 한 재리뷰어가 지목받은 위험을 "코드를 읽어 보니 괜찮다"는 주장만으로 통과시켰고, 내가 직접 시뮬레이션해서야 회귀가 드러났다(D-12). 이후 재리뷰 지시에는 **검증 방법까지** 적었다 — "파이썬으로 직접 구성해 두 척도를 계산하고 비교하라".
