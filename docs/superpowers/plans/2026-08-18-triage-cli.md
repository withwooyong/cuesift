...
def _format_triage_summary(
    ...
    **요청 예산과 실제 비율을 함께 낸다.** hard fail이 quota를 소진하므로
    ...
    lines = [
        ...
        # `cli.py:1119-1123`이 그것을 이 저장소의 **1급 결함**으로 이미 적어 두었다 -
        # "검수자가 위반 목록을 눈앞에 두고 요약만 보면 '0%니까 통과'로 읽는다."
        # 트리아지 요약에도 같은 위험이 있다: 검수 대상이 하나라도 있으면 0%로
        # 보여선 안 된다. `_format_ratio`는 그 경우 `"<0.1%"`를 낸다.
        #
...
- Modify: `docs/요구사항정의서.md` **§5.6**(FR-6.x, 334줄~) · **§5.7**(FR-7.x, 342줄~) —
  상태 열을 **새로 추가**한다
...
- [ ] **Step 1: 요구사항정의서 §5.6·§5.7에 상태 열을 추가한다**

**절 번호를 확인하고 쓴다.** 실측한 §5 하위 절 구성:

| 절 | 제목 | FR |
| --- | --- | --- |
| §5.3 | 신호 수집 (Signals) — Tier 0 | FR-3.x |
| §5.4 | 신호 수집 (Signals) — **Tier 1** | FR-4.x |
| §5.5 | 자막 규격 (Spec) | FR-5.x |
| **§5.6** | **위험도 융합 및 트리아지 (Risk & Triage)** | **FR-6.x** (334줄~) |
| **§5.7** | **출력 (Output)** | **FR-7.x** (342줄~) |

> 계획 초안은 이 절들을 전부 "§5.4"라고 적었다. WP8a 세션이 §5.4(Tier 1)에만 상태 열을
> 넣었고 그 기억에서 절 번호를 잘못 일반화한 것이다. **FR 번호는 안정적이지만 절 번호는
> 문서 구조에 종속된다** — 인용 전에 확인한다.

**§5.6·§5.7 표에는 상태 열이 없다** — 열이 `ID │ 요구사항 │ 우선순위` 셋뿐이다.
따라서 "갱신"이 아니라 **열 추가**다. §5.4가 이미 쓰는 표기(✅ / 🟡 / ⬜)를 따른다.

**FR-6.3 전체는 🟡다** — ②(위험도 임계값)는 닫히지만 ①이 "상위 K개"를 빼고 부분
충족이므로 ✅로 세지 않는다. FR-5.3이 "라이브러리엔 있으나 CLI에서 도달 못함"으로 한 번
걸렸던 자리이므로 같은 함정을 반대 방향으로도 반복하지 않는다.
