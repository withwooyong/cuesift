...
> 실행 완료 후 추가 (2026-07-29). 브랜치 `feat/ted2020-benchmark-harness`, 커밋 19개, 테스트 169 → 271건.

**이 절은 위 계획과 실제 구현의 차이를 기록한다.** 위 본문의 코드 블록은 **고치지 않았다** — 최종 코드로 갈아 끼우면 계획이 "구현과 같은 것을 두 번 적은 문서"가 되고, 실제 코드가 항상 더 정확하니 존재 이유가 사라진다. **따라서 위 본문의 코드 블록은 신뢰하지 말고 `bench/`를 보라.**
...
| 계획 결함 | **12건** (P-1 ~ P-12) |
| 그중 구현 **전에** 잡은 것 | **6건** |
| fix 루프 | Task 1~8에서 **0회**, Task 9에서 3회 |
| 최종 판정 | 병합 가능 (Critical 0건) |

**fix 루프가 0회인 것은 결함이 없어서가 아니라 구현 전에 잡혔기 때문이다.**

### 결함의 발견 시점이 성격을 가른다

| 시점 | 결함 | 무엇이 틀렸나 | 무엇을 해야 보이나 |
| --- | --- | --- | --- |
| 구현 전 | P-1·P-2·P-3·P-5·P-6·P-8 | 계획의 **가정** | 실데이터를 만진다 |
| 구현 중 | P-4·P-7·P-9 | 계획의 **산수** | 코드를 돌린다 |
| 리뷰 중 | P-10 ~ P-12, I-1 ~ I-4 | 산출물의 **서술** | 문서를 읽는다 |

**마지막 단계가 가장 늦게 드러나고 가장 오래 남는다.** 코드 결함은 테스트가 잡지만 서술 결함은 사람이 읽어야 보이고, 한 번 인용되면 정정이 따라가지 못한다. 이 브랜치의 마지막 일곱 커밋이 전부 서술 교정이었다.

### 특히 값진 결함 셋

**P-3 — 배분 루프가 en-ko에서만 실패한다.** 브리프의 배분은 알파벳순 첫 미충족 유형에 배정하는데, `glossary` 자격률이 1.78%라 인덱스를 대량 소비하고 뒤 유형이 굶는다. 실트랙 10시드에서 **en-ko 10/10 실패, ja-ko 0회** — `number` 자격률이 ja-ko 11.20% vs en-ko 5.12%로 갈리기 때문이다. **언어쌍 하나로만 테스트하면 통과한다.**

**P-5 — 불변식 2가 자기 자신과 비교했다.** `random_baseline`이 `len(chosen)/n`을 재는데 `len(chosen)`은 언제나 `take`라 상수다. 그 값을 `review_ratio`와 비교하니 차이가 항상 정확히 0. **어떤 버그로도 실패하지 않는 게이트였다.** 수정의 핵심은 `measure()`와 `random_baseline()`이 **같은 `_recall` 헬퍼를 공유**하는 것이다 — 분리하면 무작위 기준선이 자기 자신만 검증한다(실측: 분모 버그에서 분리 설계는 통과, 공유 설계는 실패).

**P-8 — negation 주입이 측정을 오염시켰다.** `"not "` 4자 삽입이 폭을 en 10% / ja 20% 늘리는데 트랙 여유가 `SAFETY = 1.10`으로 정확히 10%다. 그래서 CPS 위반이 되고 `spec.violation`이 en 79.7% / ja 93.7% 발화했다. **의미 반전을 잡은 게 아니라 길이 증가를 잡은 것**이며, 그 숫자를 실으면 스펙 §5.4가 "Tier 1 투자 근거"라 못 박은 주장이 뒤집힌다.

### 실측이 계획을 바꾼 것

| 계획의 가정 | 실측 | 영향 |
| --- | --- | --- |
| 제외는 예외적이다 | **약 50%가 규격에 안 담긴다** | 표본 순서를 뒤집어야 5,000이 나온다(P-1) |
| 용어집은 "사전적 1:1"이면 된다 | 흔한 말일수록 대응률이 낮다(`의사` 56%) | 채택 기준에 **대응률**을 추가(P-2) |
| 불변식 4는 `= 0`이다 | **en 0.96% / ja 0.93%** | 임계 기반으로 완화, 오탐률을 측정값으로(P-6) |
| 일본어는 어절 사이에 공백이 없다 | TED2020의 ja는 공백으로 구분된다 | 주 경로가 문자 분할이 아니라 어절 분할 |

### 숫자를 어떻게 말하는가 — 코드보다 오래 걸린 부분

기능은 `3e80154`에서 끝났다. 이후 일곱 커밋이 전부 오독 방지다.

| 커밋 | 무엇을 막았나 |
| --- | --- |
| `dfa12ca` | `negation` Recall에 **예산 병기** — 1.41%(10%)와 9.86%(20%)가 다르다 |
| `6ccd867` | **무작위 기준선 비교** — "1.41%도 잡긴 잡네"를 차단 |
| `5e78a55` | 스펙 §4.4의 **제외율 49.91% 복원** — 표본이 짧은 문장에 편향됐다는 사실 |
| `da2a76d` | **샘플링 방법론 명시** + "구분 불가"를 계산으로 판정 |
| `1caab34` | "다른 유형으로는 정상 주입됐다"가 **산술적으로 틀림**(최대 428/4,041) |
| `62130cd` | `else` 분기 — **ja-ko의 더 강한 결과가 침묵으로 사라지는 것** |
| `b6285ca` | 리포트가 **실재하는 검출기 결함을 부인**하는 문장 삭제 |

마지막이 가장 나빴다. 리포트가 *"검출기 버그가 아니라 만·억 vs billion 때문"* 이라 단언했는데, ja-ko 오탐 41건 중 **0건**이 그 원인이었고 **31.7%가 진짜 검출기 결함**(`_NUMBER`의 NFKC 미정규화)이었다. 그리고 `assert "billion" in md`가 그 거짓 문장을 회귀 방지선으로 고정하고 있었다.

**헤드라인을 8.64x가 아니라 6.96x로 고른 것도 같은 계열이다.** 예산 1%의 배수가 더 크지만 그 구간은 실제 검수가 6.32%라 "1% 예산에 8.6배"로 읽히면 거짓이 된다. 계산은 이미 정직했지만(실제 비율로 나눔) **어느 행을 헤드라인으로 뽑느냐도 같은 정직함을 요구한다 — 가장 큰 숫자가 아니라 가장 오해가 적은 숫자를 고른다.**

### 프로세스 기록

이 계획을 실행하며 컨트롤러(나)가 저지른 실패다. 계획 A의 같은 절이 이번 실행에 실제로 도움이 됐으므로 남긴다.

| # | 실패 | 결과 |
| --- | --- | --- |
| 1 | 구현자의 `DONE`을 **종료로 읽고** 다음 구현자를 띄웠다 | 정정 메시지에 대한 후속 커밋이 오면서 잠깐 두 구현자가 같은 브랜치에 있었다. **정정을 보냈으면 그 응답까지 받고 다음을 띄웠어야 한다** |
| 2 | 지시 응답 전에 **리뷰 패키지를 만들었다** | 리뷰어가 받은 범위에 아직 구현되지 않은 것이 있어 스펙 준수 ❌를 냈다. **리뷰어 판정은 옳았고 틀린 것은 내가 준 질문이다** |
| 3 | 리뷰어의 오해를 **한쪽에서만 닫았다** | 같은 서술이 이미 산출물에 있었는데 확인하지 않았다. **리뷰어의 오해는 대개 산출물의 문구에서 온다 — 정정할 때 역추적해야 한다** |
| 4 | 상태를 말할 때 **기준 커밋을 안 붙였다** | "else 분기가 없다"고 보고했으나 이미 있었다. 비동기 에이전트에게 상태를 말할 때는 **그 상태가 언제 것인지**를 함께 말해야 한다 |
| 5 | 리뷰어 지시가 **`git stash`·기본 출력 경로를 안 덮었다** | 리뷰어가 `python -m bench.run`을 기본 `--out-dir`로 돌려 다른 에이전트의 미커밋 작업 위에 덮어썼고, `git stash`가 구현자의 편집을 가져갔다. 둘 다 무해했으나 운이었다 |
| 6 | 리포트 문장을 **en-ko 사례만 보고 일반화했다** | "만·억 vs billion" 원인이 ja-ko에서는 0%였다. 최종 리뷰어가 원인별 분류를 실측하지 않았다면 그대로 인용됐을 것이다 |

**1·2·4는 같은 문제다** — 비동기 통신에서 "지금 상태"를 말할 때 그 기준점을 빠뜨렸다.

### 리뷰가 잘 작동한 형태

- **구체적 위험 + 검증 방법을 함께 지목**하면 적중한다. "잘 봐 달라"는 승인 도장만 받아온다
- **뮤테이션 테스트**가 결정적이었다. 값 연동이라 주장한 테스트 2건이 실제로는 하드코딩을 못 잡는다는 것을 그것으로만 밝힐 수 있었다 — **값 연동만으로는 부족하고 값이 두 가지 이상이어야 한다**(픽스처가 하나면 "값에서 뽑은 것"과 "그 값을 박은 것"이 구분되지 않는다)
- **격리 워크트리**(`git worktree add --detach`)로 공유 작업트리를 보호한 것은 리뷰어가 스스로 찾아낸 대처였다
- **컨트롤러가 준 데이터에 이의를 제기한 것**이 두 번 옳았다 — 무작위 기준선의 샘플링 기준, `negation` 헤드라인의 예산 선택
