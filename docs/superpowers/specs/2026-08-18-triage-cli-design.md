...
flowchart TD
    ...
    P1 -->|하나라도 없다| H["exit 2 — LLM 비용 전에 막는다"]
    P1 -->|전부 있다| E["언어별 루프"]
...
| 자리 | 틀리면 | 게이트 |
| --- | --- | --- |
| 프로파일 사전 로드 | 앞 언어의 LLM 비용을 쓴 뒤 exit 2 | §9.4 (프로파일 사전 검증) |
...
없는 언어는 exit 2다(D7). **메시지를 새로 쓰지 않는다** — `load_builtin`이 이미 목록을
담은 `FileNotFoundError`를 던지므로(`spec/profile.py:177-180`) CLI는 그것을 전달한다.

```text
--to fr  →  exit 2
    "[fr] 'fr' 프로파일이 없다. 사용 가능: en, ja, ko, ted-en, ted-ja, ted-ko"
```

`[fr]` 라벨만 CLI가 붙인다 — `_translate_one`의 다른 `_echo` 여섯 곳이 전부
`[target_lang]`을 붙이는 관례이고, 언어가 여러 개일 때 어느 언어의 실패인지 구별해야
...
| 프로파일 없음 | `--to fr` → exit 2, `load_builtin`의 메시지가 사용 가능 목록을 담아 전달된다 |
| **프로파일 사전 검증 (D13)** | `--to en,fr` → **번역을 한 번도 호출하지 않고** exit 2. 프로바이더 호출 0회를 실측한다 — 루프 안에서만 검사하는 버전에서는 en 번역이 실행돼 죽는다 |
