...
| "상위 K개" 예산 | ~~보류~~ → **[개수 기반 검수 예산 설계](2026-09-03-review-top-k-design.md)가 닫았다 (2026-09-03)** | FR-6.3 ①의 절반. 라이브러리에 대응 함수가 없고, 이 저장소의 표준 운영점이 비율(10%)이다. **후자는 지금도 참이고 전자가 바뀌었다** - `select_by_count`를 신설해 `--review-top-k`가 붙었다 |
...
| D5 | **`--review-budget`은 비율만 받는다.** 개수(`50`)는 exit 2 | 라이브러리에 대응 함수가 없다. 환산(`k/n`)으로 흉내내면 `ceil`과 hard fail 소진 때문에 정확히 K개가 나오지 않아 옵션이 거짓말을 한다. **2026-09-03 갱신 아래 참고** |
...
> **2026-09-03 갱신 - D5의 "보류"는 닫혔다.** [개수 기반 검수 예산 설계](2026-09-03-review-top-k-design.md)가
> `--review-budget`을 건드리지 않고 **별도 옵션 `--review-top-k`** 를 신설해 닫았다.
> D5의 두 근거가 각각 이렇게 처리됐다.
>
> | D5의 근거 | 2026-09-03 판정 |
> | --- | --- |
> | "라이브러리에 대응 함수가 없다" | **무효가 됐다** - 개수 전용 함수 `select_by_count`를 신설했다. `select_by_budget`과 `_select_top` 공통 헬퍼를 공유해 hard fail 소진과 동점 처리가 두 축에서 갈리지 않는다 |
> | "`k/n` 환산은 정확히 K개를 내지 못한다" | **여전히 참이고, 그래서 환산을 하지 않았다.** `--review-budget`에 `50`을 주면 지금도 exit 2다 |
> | "hard fail 소진" | **거부가 아니라 표시로 처리한다** - 비율 축도 같은 성질이라(D8) 요약이 실제 개수와 실제 검수 비율을 함께 낸다 |
>
> **D4의 상호배타가 2자에서 3자로 늘었다** - `--review-budget`·`--review-threshold`·`--review-top-k`
> 셋 중 둘 이상을 주면 exit 2이고, `_resolve_exclusive`가 N자로 일반화됐다.
> **`--review-top-k`는 `--tier1`과 함께 쓸 수 없다**(새 설계 D2, 이월).
...
> **2026-09-03 갱신 - 이 표는 이 설계 직후의 상태다.** 그 뒤 세 행이 움직였고,
> 이 문서는 그 시점의 판정을 남기는 기록이므로 표는 고치지 않고 아래에 후속만 적는다.
>
> | 행 | 이 설계 직후 | 지금 | 무엇이 닫았나 |
> | --- | --- | --- | --- |
> | FR-6.3 ① | 🟡 비율만 | **✅** | [개수 기반 검수 예산 설계](2026-09-03-review-top-k-design.md)의 `--review-top-k` (2026-09-03) |
> | FR-6.4 | 🟡 집계만 | ✅ | FR-7.2 `review.json` (2026-08-19) |
> | FR-7.4 | 🟡 소요 토큰 없음 | ✅ | WP8b가 Tier 1 토큰을 `cost`에 합산 (2026-08-27) |
>
> **따라서 "FR-6.3 전체는 🟡로 남는다"는 2026-09-03에 무효가 됐다.** 판정의 단일 출처는
> [요구사항정의서](../../요구사항정의서.md) §5.6이고, 개수의 출처는 [WBS](../../WBS.md)다.
...
| [요구사항정의서](../../요구사항정의서.md) **§5.6·§5.7** | FR-6.3을 🟡(①은 비율만·②는 완료), FR-6.4를 🟡(집계만), FR-7.4를 🟡로 표시한다. **두 절에는 상태 열이 아예 없어 "갱신"이 아니라 열 추가다** — 초안은 이 절들을 §5.4라 적었으나 §5.4는 Tier 1(FR-4.x)이다(Ruling 17, 아래 각주와 같은 정정) |
