...
| D4 | 판정은 **개행을 지운 사본**에 하고, CJK 는 뒤따르는 글자를 배제한다 | 자막은 어절 중간에서 줄바꿈되므로 `かもし`+개행+`れません` 이 통과한다. 경계가 없으면 「しか」가 접속사 「しかし」를 문다 (CLAUDE.md 실측 전례 2건) |
...
| --- | ---: | ---: | ---: | ---: | ---: |
| P1 원문 **또는** 번역문에 표지 | **4.60x** | **4.49x** | **3.32x** | **3.26x** | 74~86% |
| P2 원문에만 표지 | 5.17x | 4.98x | 4.98x | 4.94x | 68~81% |
| P3 원문에 있고 번역문에 없음 | 11.99x | 9.33x | 10.70x | 7.86x | 56~71% |

우선 풀 크기는 P1 기준 657~1,002건으로 **네 칸 전부에서 cap(250건)을
...
| --- | ---: | ---: | ---: | ---: |
| en-ko 10% | 17.9 | 14.3 | +20.1%p (1.41% → **21.5%**) | 10.28% |
| en-ko 30% | 18.3 | 14.6 | +20.6%p (19.72% → **40.3%**) | 30.44% |
...
| --- | --- | --- | --- |
| T1 | 개행 통과 (D4) | `かもし\nれません` 이 표지로 판정된다 | 개행 제거를 뺀다 |
| T2 | CJK 경계 (D4) | 「しかし」만 있는 문장은 표지가 아니다 | `(?!し)` 를 뺀다 |
| T3 | 한국어 띄어쓰기 변이 (D4) | 「못 했습니다」와 「못했습니다」가 모두 잡힌다 | `못\s` 또는 `못하` 중 하나를 뺀다 |
