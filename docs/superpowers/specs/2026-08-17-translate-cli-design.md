...
**이 절이 본문보다 최신이다.** 본문은 2026-08-17 설계 시점 판이라 아래와 어긋나는
자리가 있고, 그때는 **여기를 따른다.** 어긋나는 자리는 사료로 남기고 포인터만 달았다.

| 무엇 | 설계 시점 | 지금 | 언제·왜 |
| --- | --- | --- | --- |
| **U10 — 형식 위반 응답의 캐시**<br>(§3.5 표 · §3.6 · §13 U10) | HTTP 200이면 형식을 어긴 응답도 캐시한다. NFR-3은 "같은 결과"이지 "더 나은 결과"가 아니다 | **`invalid_response`는 캐시에서 폐기한다.** 재실행하면 그 세그먼트만 다시 불린다 | 2026-08-29 [파싱 실패 응답 폐기](../plans/2026-08-29-cache-discard-invalid.md)(파킹 #13)가 뒤집었다. 착수 조사가 실제 피해를 다시 쟀다 — 캐시된 실패가 **재실행을 실제 호출 0개로 만들어** "같은 설정으로 다시 돌린다"는 가장 흔한 복구 수단을 무력화하고 있었다. 경위는 [CHANGELOG](../../../CHANGELOG.md) |

**함정이 사라진 것이 아니라 한 사유로 좁혀졌다.** `empty_translation`은 지금도 캐시에
남는다 — 개수도 번호도 맞은, 계약을 지킨 응답이라 폐기하면 같은 배치에서 성공한
나머지까지 다시 결제한다. `provider_error`는 애초에 저장되지 않는다(예외가 저장 코드에
도달하지 못한다). 즉 §3.6이 서술한 함정은 **`empty_translation` 하나에만 남아 있다.**
...
> **표 첫 줄은 폐기됐다** — 위 「이후 뒤집힌 결정」 참고. 지금은 형식을 어긴 응답
> (`invalid_response`)을 캐시에서 폐기한다. 아래 본문은 설계 시점 판으로 남긴다.

예외를 캐시하지 않는 것은 **구조적으로 보장된다** — 안쪽 `complete()`가
던지면 저장 코드에 도달하지 못한다. 조건문으로 거르는 것이 아니다.

### 3.5.1 캐시 히트의 `usage`

**저장된 `usage`를 그대로 돌려준다.** 0으로 바꾸지 않는다.

| 안 | 무엇을 말하게 되나 | 문제 |
| --- | --- | --- |
| **저장값 그대로** (채택) | "이 번역에 든 토큰" | 실행에서 실제로 쓴 것과 다르다 |
| `TokenUsage()` 0 | "이번 실행에서 쓴 토큰" | `calls`가 0이 되어 "호출당 토큰"이 계산 불가. NFR-3 재현성도 깨진다 |

**실제 네트워크 호출 수는 `CachingProvider.misses`가 정확히 센다.**
요약이 그 수를 함께 내므로 두 정보가 다 남는다 — `usage`는 "이 작업의
비용", `misses`는 "이번에 실제로 지불한 비용"이다. 하나로 합치면
둘 중 하나를 영영 알 수 없게 된다.

### 3.6 그래서 생기는 것 — 캐시가 함정이 되는 지점

> **이 절은 폐기됐다** — 위 「이후 뒤집힌 결정」 참고. 형식을 어긴 응답은 폐기되므로
> 재실행하면 그 세그먼트만 다시 불린다. 함정은 `empty_translation`에만 남는다.
> 아래는 설계 시점 판으로 남긴다.
...
```text
IngestResult
  ├─ subs         : pysubs2.SSAFile      원본 그대로 (deepcopy해서 쓴다)
  ├─ event_index  : {segment_id: 원본 이벤트 인덱스}
  └─ segments     : [Segment]            target_text가 채워져 돌아온다

writer:  event = copy.events[event_index[seg.id]]
         prefix = 선행_오버라이드_블록(event.text)
         event.plaintext = seg.target_text      # \n → \N 변환을 해 준다
         event.text = prefix + event.text       # 선행 블록만 되붙인다
         copy.save(out_path, format_=result.format)
```
...
**`deepcopy`가 필요하다.** `result.subs`를 직접 고치면 `--to en,ja`에서
두 번째 언어가 첫 번째 번역 위에 덮인다.

### 5.2.1 태그는 일부만 살아남는다 [실측 2026-08-17]

`loader.py`가 `source_text=event.plaintext`를 쓰므로 번역문에는 태그가 없다.
그런데 **pysubs2의 `plaintext` setter는 기존 태그를 전부 지운다.**

| 원본 `text` | `plaintext` setter만 쓰면 | 선행 블록 보정 후 |
| --- | --- | --- |
| `{\an8}{\i1}기울임{\i0} 대사` | `번역문` | `{\an8}{\i1}번역문` |

**선행 오버라이드 블록(`^(?:\{[^}]*\})*`)만 되붙인다.** 그래서 위치
태그(`\an8`·`\pos`)와 첫 스타일 지정은 살아남고, **중간·후행 태그는 잃는다.**

| 무엇 | 살아남나 |
| --- | --- |
| 타임코드 · 스타일 이름 · 이벤트 종류(Dialogue/Comment) · 레이어 | ✅ 원본 이벤트 그대로 |
| 선행 오버라이드 블록 (`{\an8}`·`{\pos(x,y)}`) | ✅ 되붙인다 |
| VTT의 여러 줄 (`\N` ↔ 실제 줄바꿈) | ✅ setter가 변환한다 |
| 중간·후행 태그 (`{\i0}` 닫는 태그 등) | ❌ **잃는다** |
| 입력 파일의 UTF-8 BOM | ❌ **출력엔 없다** [실측 2026-08-17]. 재읽기는 정상이지만 BOM으로 인코딩을 판별하는 일부 Windows 자막 도구에는 영향이 있을 수 있다 |
| 번역문에 섞인 `{...}` (LLM 출력은 통제 밖) | ❌ **잃는다** [실측 2026-08-17]. srt/vtt는 저장 시점에 지워지고, ass/ssa는 파일엔 남지만 재생 시 오버라이드 블록으로 해석돼 화면에서 사라진다. ko→en/ja에서 드물어 코드는 고치지 않았다 |

**완전 보존은 불가능하다.** 번역문의 어느 위치에 중간 태그를 넣을지
결정할 근거가 없다 — 원문의 "기울임" 3글자에 걸린 강조가 번역문
"italic"의 어디에 걸리는지는 정렬 정보 없이 알 수 없다. `.srt`·`.vtt`는
태그가 거의 없어 실질 영향이 작고, `.ass`는 영향을 받는다.

**닫는 태그를 잃는 부작용은 자막 하나 안에 갇힌다** — 다음 이벤트는
스타일을 새로 시작하므로 강조가 자막 전체로 번지지 않는다.
...
"한 실행에 여러 언어"로 읽었고, `Segment.target_text`가 단수다.

**구현은 `IngestResult`를 언어마다 다시 읽지 않는다 — 한 번만 읽고
공유한다.** [실측 2026-08-17, 최종 리뷰 D-1] `load_subtitle`은
`translate()` 진입 시 한 번만 불리고(`cli.py`), 그 결과 `result`를 언어
루프가 그대로 재사용한다. 안전한 이유는 이 계층이 아니라 아래 계층에
있다 — `translate/engine.py`가 `replace(s, target_text=...)`로 **새
튜플**을 만들어 돌려주므로 원본 `Segment`는 변형되지 않는다(`_translate_one`
독스트링에 근거가 있다). 그래서 여러 언어를 돌아도 앞 언어의 번역문이
다음 언어로 새지 않는다 — 다시 읽지 않아도 되는 이유이지, 다시 읽는다는
뜻이 아니다.
...
[en] ./ep01.en.srt
  ...
  캐시 히트 64개 · 호출 필요 18개 이상
  프롬프트 문자 system 1,204 + user 31,880

(토큰·비용은 내지 않는다 - 문자에서 토큰으로 가는 계수의 출처가 없다)
```

[실측 2026-08-17, 최종 리뷰 D-3] 위 예시는 실제 코드(`_dry_run_report`)의
출력을 그대로 옮긴 것이다 — Task 6 리뷰가 "호출 필요 N개"에 **"이상"을
붙이는 것**(하한임을 표시)과 출력 경로를 별도 줄이 아니라 `[en]` 헤더
줄에 넣는 것으로 이 스펙 초안에서 바뀌었는데, 본문 예시가 그 변경을
따라오지 못했었다. 대상 언어가 여럿이면(`--to en,ja`) 이 `[en]`/`[ja]`
블록이 그대로 반복된다.
...
| 라운드트립 | 픽스처 전부 — 타임코드·스타일·이벤트 종류가 보존되고 **선행 태그 블록이 되붙는지**. §5.2.1의 표가 기대값이다 |
...
| 실제 `.ass` 자막에서 태그 소실의 체감 영향 | §5.2.1은 무엇이 남고 무엇이 사라지는지만 확정했다. 실제 작업물로 확인한 적이 없다 |

### 12.1 이 설계가 발견했지만 고치지 않는 것

**`{\p1}` 벡터 드로잉 이벤트가 번역 대상으로 들어간다** [실측 2026-08-17].
`tests/fixtures/ingest/tags.ass`의 3번째 이벤트가 그 경우다 —
`plaintext`가 `m 0 0 l 100 0 100 100 0 100 b 20 20 ...` 좌표 문자열인데
`loader.py`는 이것을 일반 자막과 구분하지 않는다. LLM에 보내면 번역을
시도하고, 결과는 깨진 드로잉이다.

**이번 범위에서 고치지 않는다.** 판정은 `_keep_displayed`(인제스트 계층)의
몫이고, 고치려면 "무엇이 표시되는 자막인가"의 정의를 바꿔야 한다 —
FR-1.1의 계약 변경이라 요구사항정의서를 먼저 봐야 한다.

`translate`가 처음으로 이 결함을 **사용자에게 보이게 만든다**는 점은
기록해 둔다. `check`는 드로잉을 규격 위반으로 판정할 뿐 손대지 않았다.
...
| U21 | **선행 오버라이드 블록만 되붙인다** | 완전 보존은 불가능하다 — 중간 태그를 번역문의 어디에 넣을지 결정할 근거가 없다. 위치 태그를 잃는 것이 더 나쁘다 |
| U22 | **`subs`를 `deepcopy`해서 쓴다** | 직접 고치면 `--to en,ja`에서 두 번째 언어가 첫 번째 번역 위에 덮인다 |
| U23 | **캐시 히트도 저장된 `usage`를 그대로 낸다** | 0으로 만들면 `calls`가 0이 되어 "호출당 토큰"이 계산 불가. 실제 호출 수는 `misses`가 따로 센다 |

> **U10은 폐기됐다** — 위 「이후 뒤집힌 결정」 참고. 2026-08-29에 뒤집혀 지금은
> `invalid_response`를 캐시에서 폐기한다. **이 결정 로그를 근거로 삼기 전에 그 절을
> 먼저 읽어야 한다** — 나머지 항목은 유효하다.
