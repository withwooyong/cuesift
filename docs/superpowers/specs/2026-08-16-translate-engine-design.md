...
| 변이 | 실측 결과 | 죽는가 |
| --- | --- | --- |
| 마커 미등록 | `Failed: 'live' not found in markers` → `Interrupted: 1 error during collection`, exit 2. **나머지 839개가 실행조차 안 된다** | 요란하게 |
| `-m "not live"` 삭제 | 키 없는 환경에서 `deselected`가 `skipped`로 바뀔 뿐. CI는 그대로 초록 | **조용히 — 안 죽는다** |
| live 모듈의 `pytestmark` 삭제 | 위와 동일 | **조용히 — 안 죽는다** |
| `pytest.ini` 추가 | pytest가 pyproject보다 그쪽을 **우선**해 우리 설정이 통째로 무시된다 | **조용히 — 안 죽는다** |
| `addopts` 끝에 `-m live` | 뒤의 `-m`이 이겨 `1 skipped, 832 deselected`. exit 0 | **조용히 — 안 죽는다** |
| `--strict-markers` 삭제 | 이 표 1행의 전제가 사라진다 | **조용히 — 안 죽는다** |
| 마커 없는 **두 번째** live 파일 | 그 파일이 기본 수집에 들어와 그대로 실행된다 | **조용히 — 안 죽는다** |
...
훅은 수집보다 먼저 돌아 그 대상이 아니다. 실측: 위 표의 1a·1b·1c에 해당하는
셋이 지금은 **exit 4로 수집 전에 서고**, 네 번째(두 번째 live 파일)는 테스트
1건을 죽인다.
...
| `pytest --cov=cuesift` | **수집 개수 증가분을 기록한다.** `translate/` 커버리지 확인 | ✅ **839 passed, 1 deselected** (착수 시 499 → Task 6까지 813). `translate/` 6개 모듈 **전부 100%**, 전체 99% |
...
`1/840 tests collected (839 deselected)`가 되어 live 쪽이 실재함을 확인할 수 있다.
