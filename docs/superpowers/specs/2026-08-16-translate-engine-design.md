...
`pytest.ini` 추가)는 지나간다 — 실측 exit 0, 841 deselected. pyproject의
...
마지막 행이 남은 구멍이다. 닫으려면 ①이 `config`를 봐야 하는데 임포트
시점에는 `config`가 없다. **두 변경을 동시에 해야 하는 조합이라 사고보다
의도에 가깝고, 그 사실을 적어 두는 편이 없는 척하는 것보다 낫다.**
