...
| # | 변이 | 실측 결과 | 측정 시점 | 죽는가 |
| --- | --- | --- | --- | --- |
| M1 | 마커 미등록 | `Failed: 'live' not found in markers` → `Interrupted: 1 error during collection`, exit 2. **나머지 841개가 실행조차 안 된다** | **현재** (재현됨) | 요란하게 |
...
그래서 실행 결과가 아니라 **설정 그 자체**를 단언한다. 자리는 둘로 나뉜다.

| 무엇을 | 어디서 | 왜 거기인가 |
| --- | --- | --- |
| ini 파일 정체 · `--strict-markers` · `addopts`의 `-m` 개수 | `tests/conftest.py`의 `pytest_configure` | **deselect될 수 없다** |
| `markers` 등록 · live 파일들의 `pytestmark` · 훅이 실제로 거부하는가 | `tests/test_translate_api.py` | 훅이 보지 않거나, 훅 자신을 지켜야 하는 것 |

**설정 검사를 테스트에 두면 자기를 무력화하는 변이에 같이 쓸려 나간다** —
`-m live`를 덧붙이면 감시자 자신이 832개와 함께 deselect되고 exit 0이 난다
(M5, 훅 이전 실측). 훅은 수집보다 먼저 돌아 그 대상이 아니다.

**방어가 세 겹인 이유는 M9다.** 훅을 개명해 배선을 끊으면(M8) `-m live`
덧붙이기가 되살아나고, 그것을 감시하는 등록 테스트마저 함께 deselect된다.
모듈 최상위 코드는 **개명할 이름도 없고 deselect 대상도 아니라서** 그 조합에서
유일하게 살아남는다.
...
| 변이 | 지금 |
| --- | --- |
| M4 `pytest.ini` · M5 `-m live` · M6 `--strict-markers` | **exit 4**, 수집 전 중단 |
| M7 · M3 마커 없는 live 파일 | 테스트 **1건 사망** |
| M8 훅 개명 | 테스트 **1건 사망** (배선 검사) |
| M9 M8+M5 | **exit 4** (임포트 시점 검사) |
...
| `pytest --cov=cuesift` | **수집 개수 증가분을 기록한다.** `translate/` 커버리지 확인 | ✅ **841 passed, 1 deselected** (착수 시 499 → Task 6까지 813). `translate/` 6개 모듈 **전부 100%**, 전체 99% |
...
`1/842 tests collected (841 deselected)`가 되어 live 쪽이 실재함을 확인할 수 있다.
