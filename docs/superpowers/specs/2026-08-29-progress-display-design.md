...
v0.1 완료 개수는 **36 → 37**이 되고 WP6은 🟡 → ✅가 된다. FR-8.3은 WP9로 넘어간다.
...
| **R3** | Windows 콘솔에서 `\r`이 기대대로 동작하지 않을 수 있다 | 개발 환경이 Windows다. 실물 확인을 태스크로 둔다. `\r`은 ANSI가 아니라 ASCII 제어문자라 cmd·PowerShell·Windows Terminal 모두 지원하지만, **확인하지 않은 것을 확인했다고 적지 않는다** |
