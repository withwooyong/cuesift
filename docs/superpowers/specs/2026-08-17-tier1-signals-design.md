...
`translate_segments()`를 그대로 재사용한다 — 배치·컨텍스트 윈도우·재시도가
이미 구현돼 있고, 이 설계가 다시 만들 이유가 없다.

**후보만 넘기므로 앞뒤 맥락이 원본과 다르다.** 후보는 트랙 전체에 흩어져 있는데
`iter_batches`는 넘긴 목록의 앞뒤로 윈도우를 만들기 때문이다. 그럼에도 측정이
유효한 것은 **N회가 모두 같은 조건을 받기 때문**이다 — 분산은 맥락의 절대적
정확성이 아니라 같은 입력에 대한 출력의 흔들림을 잰다.
