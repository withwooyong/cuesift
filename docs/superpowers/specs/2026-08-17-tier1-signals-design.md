...
`negation` Recall 1.41%(무작위 9.61%)가 이 투자의 근거다(§3.1).
...
### 3.1 Tier 0의 원리적 한계 (2026-07-29, TED2020)

근거: [`bench/results/en-ko-2026-07-29.md`](../../../bench/results/en-ko-2026-07-29.md) ·
[`bench/results/ja-ko-2026-07-29.md`](../../../bench/results/ja-ko-2026-07-29.md)

| 예산 | `negation` Recall (en-ko) | 무작위 기준선 | 비고 |
| --- | --- | --- | --- |
| 1% | 0.00% | 5.99% ±2.63% | 전혀 못 잡는다 |
| 10% | 1.41% | 9.61% ±3.32% | **무작위의 15%** |
| 20% | 9.86% | 19.49% ±5.17% | 무작위의 51% |
| 30% | 29.58% | 29.68% ±5.92% | 무작위와 구분 불가 |

같은 재측정에서 전체 Recall은 예산 10%에서 69.60% → 72.40%로 올랐으므로
**의미 반전만 선택적으로 제자리**다. 융합식(noisy-or 전환)이나 개별 검출기를
고쳐도 소수점까지 동일했다 — 신호를 어떻게 조합하든 문법적으로 완벽한 문장의
...
> 사실에 맞춰 정정한다. **문자 수는 이 절에 적지 않는다** — 입력 문장
> 길이에 의존해 여기 별도로 적으면 재현 시도마다 다른 값이 나와 문서가
> 갈라진다(실제로 이 절의 옛 수치를 독립 재현했더니 문자 수만 어긋났다 —
> 호출 배수는 정확히 재현됐다, Task 6 재리뷰). 구체적 문자 수는
> `src/cuesift/signals/llm.py`의 `_retranslate` 독스트링이 단일 출처다.
...
| 이 설계 §6.3 | "배치·컨텍스트 윈도우를 재사용한다"가 §4.1의 세그먼트 단위 프로토콜과 모순 — 재사용되는 것은 재시도·실패 분류뿐임을 실측(호출 10배: 30회 vs 3회)과 함께 정정 (Task 5 리뷰, Ruling P11). **문자 수(옛 수치 10320자/1599자)는 뺐다** — 재리뷰 독립 측정이 문자 수만 재현 안 됨을 확인해(호출 배수는 재현됨) 입력 의존 수치를 이 절에서 없애고 `signals/llm.py`의 `_retranslate` 독스트링을 단일 출처로 가리켰다 (Task 6 재리뷰, Task 7 Item ⑧) |
