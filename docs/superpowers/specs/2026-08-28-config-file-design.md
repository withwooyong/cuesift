...
| **D5** | 타입 변환과 값 검증은 **click에 맡긴다.** 로더가 판정하는 것은 `llm.provider`와 **`signals.weights`의 값 타입** 둘뿐 | 로더가 따로 검증하면 같은 규칙이 두 곳에 생기고 반드시 갈린다. click은 `default_map` 값도 파라미터 타입으로 변환·검증한다(§3 실측). **`signals.weights`만 예외인 것은 그것이 CLI 파라미터가 아니어서 click을 거치지 않기 때문이다**(D6) — `spec.violation: "높음"`을 쓰면 `fuse()`의 `math.isfinite(weight)`가 `TypeError`를 내고, 미처리 traceback은 종료 코드 1("규격 위반 발견")로 오보된다. 범위(음수·NaN·inf)는 `fuse()`가 이미 막으므로 로더가 다시 보지 않는다 |
...
## 5. 매핑표 — YAML 24행 → 파라미터 23개

**개수의 산수를 남긴다.** 커맨드별 옵션은 `translate` 19 · `check` 3 · `transcribe` 1로 **23개**이고, 위치인자 `input` 2개는 제외한다(D13). YAML 24행 중 2행(`llm.provider`·`signals.weights`)은 파라미터로 가지 않으므로 매핑 행은 22행이며, 그중 1행(`source_lang`)이 두 커맨드에 뿌려져 23개가 된다.

`transcribe`는 아직 스텁이지만(§1.3) 한 칸을 지금 잡아 둔다. 배선(FR-8.3) 때 설정 표를 다시 여는 것보다 싸고, 빠뜨리면 "모든 옵션"이 그 시점에 조용히 거짓이 된다.
...
**범위 검증은 `fuse()`가 한다.** 음수·NaN·inf는 이미 거기서 막힌다(`fuse.py:61`). 로더가 다시 검사하면 규칙이 두 곳이 된다(D5와 같은 이유).

**그러나 값의 타입은 로더가 본다.** 이 값들은 click을 거치지 않는 유일한 항목이라, 숫자가 아니면 `fuse()`의 `math.isfinite`가 `ValueError`가 아니라 `TypeError`를 내고 그것은 미처리 traceback이 되어 종료 코드 1로 오보된다. **`bool`을 먼저 막는다** — `bool`은 `int`의 하위형이라 `float()`에 통과하고, `spec.violation: true`가 1.0이 되면 "가중치를 껐다"고 믿은 사용자가 1.0으로 검수받는다.
...
| # | 위험 | 완화 |
| --- | --- | --- |
| R1 | click 내부 동작(`default_map`)에 의존한다. typer 업그레이드가 깨뜨릴 수 있다 | 의존성은 고정이다(런타임 4개). P1~P3과 **P4의 검증 절반**은 테스트가 직접 확인한다. **P4의 변환은 확인하지 않는다** - 아래 정정 참고 |
...
| R5 | 24행 매핑표가 CLI 옵션 추가 시 뒤처진다 | 매핑표에서 허용 키를 파생시키므로(§4.1) 옵션이 늘면 설정에서만 빠진다. 이것을 잡는 테스트를 둔다 — **click 명령 객체에서 파라미터 이름을 뽑아** 위치인자를 뺀 집합이 매핑표의 대상 집합과 **정확히 같은지** 본다. 부분집합이 아니라 상등이어야 한다: 한쪽 방향만 보면 매핑표에 남은 죽은 행을 못 잡는다 |

**R5의 테스트가 이 설계에서 가장 중요한 자동 게이트다.** 사람이 표를 갱신하는 것을 잊는 것은 확실히 일어나고, 잊으면 "설정 파일로 모든 옵션을 지정할 수 있다"가 조용히 거짓이 된다.

### 9.1 R1 정정 - 완화책은 P4의 절반만 덮는다 (2026-08-28 실측)

**위 표의 이전 판은 "P1~P4의 성질을 테스트가 직접 확인한다"고 적었고 그것이 실제보다 넓었다.**
구현 후 리뷰가 `type_cast_value`를 패치해 실측했다.

| P4의 두 성질 | 게이트가 잡나 | 무엇이 잡나 |
| --- | --- | --- |
| **검증** - 틀린 값 `"xml"`이 종료 코드 2로 거부된다 | ✅ 잡는다 | 잘못된 `review.format`·`spec.fail_on`을 넣는 테스트 |
| **변환** - `"./x/y"`가 `Path`로, `"html"`이 `ReviewFormat`으로 바뀐다 | ❌ **못 잡는다** | 아무것도 없다 |

typer가 `default_map` 값의 **타입 변환만** 그만두는 회귀에서 `output.dir`이 `str`로 흘러도
**죽는 테스트가 하나도 없다.** v0.1에서는 무해하다 - 그 값을 받는 코드가 `str`도 받아
넘긴다. **그러나 완화책을 실제보다 넓게 적어 두는 것이 위험이다**: 다음 사람이 "게이트가
잡는다"를 읽고 `Path` 전용 메서드를 쓰면, 그때 처음 터지는데 원인은 typer 업그레이드라
한참 떨어진 곳에 있다.

**지금 이것을 게이트로 만들지 않는다.** 잡으려면 설정만 준 실행에서 파라미터의 **런타임
타입**을 단언해야 하고, 그것은 click 내부에 한 겹 더 의존하는 테스트다 - R1이 걱정하는
바로 그 의존을 늘린다. **적어 두는 것이 지금의 대응이다.**
