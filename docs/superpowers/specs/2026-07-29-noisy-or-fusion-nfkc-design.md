...
3·5·6·8번은 현재도 통과한다. **그래서 더 중요하다** — 이 넷이 없으면 A1·A2가
기존 계약을 깨뜨려도 드러나지 않는다.

2번을 1번과 별도로 두는 이유: 단조성만 검사하면 "모든 점수를 `max()`로 합치는"
구현도 통과한다. 구체값이 산식을 고정한다.

3번이 PASS인 것은 **두 산식이 `w=0`에서 실제로 일치하기 때문이다.** 가중 평균은
분자에 `0 × s = 0`, 분모에 `0`을 더해 그 신호를 평균에서 제외하고, noisy-or는
`(1 - s)^0 = 1`이라 곱에서 제외한다. 실측으로 확인했다 — `[a=1.0(w=0), b=0.6(w=1)]`이
현재 코드에서 `0.6`을 낸다.

### 5.1 교체해야 하는 기존 테스트

**신규 테스트만 세면 안 된다.** `tests/test_risk_fuse.py`의 2건이 가중 평균을
**값으로 고정**하고 있어 산식을 바꾸면 실패한다.

| 테스트 | 현재 기대값 | noisy-or | 조치 |
| --- | --- | --- | --- |
| `test_weighted_average_of_multiple_signals` | 0.5 | **0.84** | 이름·독스트링·기대값 교체 |
| `test_weights_shift_the_result` | 0.75 | **1.0** (`b=1.0`이 포화) | 입력을 판별력 있는 값으로 바꾼다 |

두 번째는 기대값만 고치면 안 된다 — `b=1.0`이면 `(1-1.0)^w = 0`이라 **어떤 `w`를 줘도
1.0이 나와** 가중치 검증력이 사라진다. 입력을 `a=0.5, b=0.5, w=1/3`으로 바꾸면 0.9375다.

**실패를 보고 기대값만 고치는 것은 교체가 아니다** — 무엇을 계약으로 삼는지가
기록되지 않는다. 독스트링에 "왜 이 값인가"를 함께 쓴다.
