...
### 5.4 `empty`가 이 프로젝트의 규율에 직접 대응한다

pysubs2는 자막이 아닌 텍스트도, 빈 문자열도, 타임코드 없는 SRT도 **예외 없이 0큐**로
반환한다(§12). CLAUDE.md의 **"0개 수집은 통과가 아니라 설정 오류다"** 가 정확히 이
지점이다. pysubs2가 판단하지 않으므로 인제스트가 대신 판단한다.
...
`overlap.vtt`가 생기면 HANDOFF의 미결 항목이 닫힐 준비가 된다. 지금까지는
`build_track`이 겹침 0건으로 트랙을 합성하고 주입기의 `spec` 유형은 duration을 줄일 뿐이라
**겹침을 만들지 못했다.** 기여도 `+0.0%`는 "쓸모없다"가 아니라 "측정하지 못했다"였다.
