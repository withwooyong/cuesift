...
| — | **결함 ②(비대칭 밀어냄)의 구조적 수정**(HANDOFF 이월 23번). 같은 항목의 앞 두 갈래는 **2026-09-12에 닫혔다** — `render_tier1_comparison`에 전체 Recall 행을 더했고(순증 건수·실제 검수 비율 포함), ja-ko 대조로 "예산 구간별 이득 판정"의 전제를 실측했다. **ja-ko에서는 같은 부호가 나오지 않는다** — 예산 10%의 전체 순증이 en-ko는 -2건인데 ja-ko는 +0건이라, 예산만으로 "Tier 1을 끌 구간"을 정할 수 없다. 남은 것은 결함 ② 하나이고 착수의 첫 질문은 「나머지 부류 순증이 예산의 함수인가」다(두 언어쌍 네 칸에서 예산 10%는 -4건 · 30%는 0건으로 언어쌍과 무관했다 — 다만 네 칸은 네 칸이다). FR-4.2 완료를 막지 않는다 | v0.2 |
