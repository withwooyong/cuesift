...
> 갱신: 2026-07-29 (KST) · 기준 커밋 `411b14d`
...
| **3** | 융합·검출 정정 | 6.1 · 3.4 | 🔨 | S | 1·2 | `fuse.py` noisy-or · `_numbers()` NFKC · 재측정 |
| **4** | 인제스트 | 1.1 · 1.3 · 1.5 | ⬜ | M | — | `src/cuesift/ingest/` — SRT·WebVTT·ASS 파서 |
...
flowchart TD
    ...
    WP3["WP3 융합·검출 정정<br/>🔨 진행 중"]
    WP4["WP4 인제스트<br/>FR-1.1·1.3·1.5"]
    ...
    style WP3 fill:#fef7e0,stroke:#f9ab00
```

**WP4가 병목이다** — 출력·번역·STT 셋이 전부 여기에 걸린다.
자막 파일을 `Segment`로 바꾸지 못하면 Tier 0 엔진에 실데이터가 들어가지 않는다.
...
| 순 | WP | 이유 |
| --- | --- | --- |
| 1 | WP3 | 벤치 컨텍스트가 살아 있을 때 끝낸다. 지금 안 하면 재측정 비용이 커진다 |
| 2 | WP4 | 병목. 셋이 여기 걸려 있다 |
| 3 | WP6 부분 (FR-8.2 `check`) | 최초로 **쓸 수 있는 제품**이 나온다 |
| 4 | WP7 → WP8 | Tier 1은 번역 계층 없이는 만들 수 없다. Q4가 여기서 닫힌다 |
| 5 | WP5 → WP6 전체 | 리포트와 나머지 서브커맨드 |
| 6 | WP9 | STT는 FR-1.3이 "자막 우선"이라 마지막이어도 S1이 성립한다 |
