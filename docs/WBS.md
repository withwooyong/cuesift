...
| 1 | WP7 → WP8 | Tier 1은 번역 계층 없이는 만들 수 없다. **Q4(자가일관성 유사도)가 여기서 닫힌다** — 남은 미결정 하나이며, `negation` Recall이 무작위보다 낮다는 실측이 이 투자의 근거다 |
| 2 | WP5 나머지 (FR-7.1~7.4) | `review.json`·`report.html`·요약 통계. **WP7이 먼저인 이유**는 리포트에 실을 신호가 번역 계층에서 나오기 때문이다 — 지금 스키마를 못 박으면 규격 위반 7종만 아는 상태에서 FR-7.2를 확정하게 된다(설계 D4가 `--json`을 미룬 것과 같은 판단) |
| 3 | WP6 나머지 (FR-8.1·8.3~8.5) | `translate`·`transcribe` 배선과 `cuesift.yaml` 로더·진행 표시 |
