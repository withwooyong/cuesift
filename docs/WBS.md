...
### 30 → 32 → 34 → 35 → 36 → 37 → 39 → 40 → 41 - 산수를 남긴다
...
| 축 | 대상 | 완료 |
| --- | --- | --- |
| 상태 열 **있는** FR | 요구사항정의서 §5.4·§5.6·§5.7·**§5.8**의 **18개** | **17** - FR-4.1·4.3·6.1·6.2·**6.3**·6.4·6.5·7.1·7.2·7.3·7.4·7.5·8.1·8.2·8.3·8.4·8.5 (남은 하나는 FR-4.2 🟡 - 2026-09-05, ✅는 아니라 이 17개 집계는 움직이지 않는다) |
...
`24 + 17 = 41`이 WP 축의 합과 같은 값을 낸다. **분모가 13/29에서 18/24로 옮겨간 것은
...
| 1 | v0.1 잔여 - **FR-4.2 역번역**(🟡, 2026-09-05) | 42개 중 41개가 닫혀 있고 **FR-4.2는 미완료로 남았지만 ⬜에서 🟡로 바뀌었다.** 신호 자체는 구현되고 판별력도 실측됐다(negation 역번역 코사인 0.69 대 0.78, 방향 올바름) — 그러나 Tier 1을 실제로 붙이면 Recall이 **떨어진다**(예산 10% negation Recall 1.41% → 0.00%, 예산 30% 19.72% → 18.31%). 원인은 신호가 아니라 `select_tier1_candidates`의 후보 선정이 Tier 0 위험도 순위에 의존해 의미 반전을 무작위 위치에 놓기 때문이다 — en-ko·ja-ko 두 예산 네 칸 전부에서 회색지대 안 순위 중앙값이 절반 지점이고 cap 적중이 무작위 기대값 이하였다. **완료로 올릴 근거가 아니라 다음 작업(Tier 1 후보 선정 재설계)의 착수 근거가 생겼다.** 상세: 요구사항정의서 §5.4 FR-4.2 행, [HANDOFF.md](../HANDOFF.md) 이월 21번, [`bench/results/en-ko-2026-09-05.md`](../bench/results/en-ko-2026-09-05.md) |
