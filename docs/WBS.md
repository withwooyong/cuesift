...
> 갱신: 2026-09-02 (KST) · 기준 커밋 `d390dc1`
...
`14`를 믿으면 아래 합이 **38**이 되어 표제 39와 어긋난다.

| 축 | WP1 | WP4 | WP5 | WP6 | WP7 | WP8a·b | WP9 | 합 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 전수 | 16 | 3 | 5 | 5 | 8 | 3 | 2 | **42** |
| 완료 | **15** | 3 | **5** | **4** | 8 | **2** | **2** | **39** |
| 완료(낡은 `14`) | 14 | 3 | 5 | 4 | 8 | 2 | 2 | **38 ✗** |
...
`24 + 15 = 39`가 WP 축의 합과 같은 값을 낸다. **분모가 13/29에서 18/24로 옮겨간 것은
...
flowchart TD
    ...
    WP6["WP6 CLI 배선<br/>🟡 FR-8.1·8.2·8.4 완료<br/>+ FR-6.3 CLI 배선"]
...
| 1 | WP6 나머지 (**FR-8.3** `transcribe` 배선) | **선행이 방금 풀렸다** — WP9가 그 명령이 부를 어댑터를 냈다. 배선 시점에 함께 볼 위험은 설계 스펙 §9.1의 **C1 하나다** — `_output_path`가 `talk.en.mp4` 안에 SRT를 넣는 **조용한 실패**이고, `--media`를 붙이는 커밋이 반드시 함께 고쳐야 한다. 함께 적혀 있던 C2(`_reject_non_subtitle`의 낡은 문구)는 **2026-09-02 최종 픽스 라운드 1에서 이미 닫혔다** |
