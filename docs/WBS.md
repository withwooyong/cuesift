...
| ~~13~~ | ~~WP9 (STT 어댑터)~~ | ✅ 완료 (2026-09-02). FR-1.2·1.4를 닫고 FR-1.3의 반쪽을 채웠다. **착수 시점 기록은 "Whisper 계열 어댑터"였고 그것이 틀렸다** — 설계 D1이 파이썬 STT 패키지를 배제해 실제로는 OpenAI 호환 HTTP 어댑터가 됐다. 테스트 1582 → **1678**. [설계 스펙](superpowers/specs/2026-08-30-stt-adapter-design.md) |
| 1 | WP6 나머지 (**FR-8.3** `transcribe` 배선) | **선행이 방금 풀렸다** — WP9가 그 명령이 부를 어댑터를 냈다. 배선 시점에 함께 볼 위험 둘이 설계 스펙 §9.1에 있다: `_output_path`가 `talk.en.mp4` 안에 SRT를 넣는 **조용한 실패**(C1)와 `_reject_non_subtitle`의 "STT는 v0.1에 없다"가 거짓이 되는 것(C2) |
