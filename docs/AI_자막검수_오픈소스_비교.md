...
> **동기화 기준일: 2026-07-28.** 이 절은 [요구사항정의서](요구사항정의서.md) §10(로드맵)·§12(미결정 사항 Q1~Q6)의 결정을 반영해 갱신했다. 조사 문서는 조사 시점에 멈추기 쉬우므로, 요구사항 쪽에서 결정이 날 때마다 이 절을 다시 맞춘다.
...
- **평가 데이터**: K-드라마 자막은 저작권 때문에 리포에 넣을 수 없다. → §9.2에서 **TED2020/OPUS + 합성 오류 주입**으로 확정됐고, 측정 오염을 막기 위해 벤치마크 전용 `specs/ted.yaml`을 분리하기로 했다(Q5).
...
| 2 | xCOMET 없이 **경량 신호만으로** 트리아지가 성립하는가? | 🔬 **측정 필요** | **살아 있는 유일한 질문.** 미결정 **Q4**(자가일관성 유사도 측정 수단)와 같은 실험에서 답이 나온다. 선행 조건은 TED2020 ko-en·ko-ja 코퍼스 확보 |
...
`★ 정리 — 지금 필요한 것은 조사가 아니라 측정 ──`
6개 중 5개는 **닫혔거나, 로드맵상 아직 시점이 아니거나, 의사결정에 쓰이지 않는다.** 남은 #2는 남의 리포지토리를 열어봐서 답이 나오는 질문이 아니다 — ko→en/ja 병렬 코퍼스에 **합성 오류를 주입해 Recall@Budget을 실측**해야 한다(§9.1·§9.2).

**따라서 다음 작업은 웹 조사가 아니라 ① TED2020 ko-en·ko-ja 확보 → ② 측정 하네스 작성 → ③ Q4·#2 동시 해결이다.**

조사 질문을 적을 때 **"조사로 닫히는 것"과 "측정으로 닫히는 것"을 섞지 않는 편이 낫다.** 섞이면 측정해야 할 항목이 조사 목록에 묻혀 계속 미뤄진다 — #2가 실제로 그랬다.
`─────────────────────────────────────────────`
