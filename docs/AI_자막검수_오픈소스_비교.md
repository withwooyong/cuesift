...
- 🟡 표기 항목(subsai·TransQuest·OpenKiwi·pysubs2·Subtitle Edit의 라이선스·스타·최신 릴리스)은 검색 결과 기반이며 **리포 직접 확인 미완료**. 다만 이 중 subsai·TransQuest·OpenKiwi는 §4에서 **채택 후보가 아니므로** 착수를 막지 않는다.
...
| 4 | pysubs2의 TTML 지원 깊이 — IMSC1까지 되는가? | ⏸ **연기** | `FR-1.6`(TTML 입출력) = **v0.3**. v0.1 필수는 `FR-1.1`의 SRT·WebVTT·ASS/SSA |
