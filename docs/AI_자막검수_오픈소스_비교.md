...
>
> **2026-07-29 갱신**: TED2020 벤치마크 실측으로 아래 #2가 닫혔다. §12 Q4는 이 실측으로 **열린 채** 남는다 — 두 질문이 다른 계층(#2는 Tier 0, Q4는 Tier 1)을 묻기 때문이다. 자세한 내용은 이 절 하단 표와 요구사항정의서 §12 Q4를 참고.
...
**① TED2020 ko-en·ko-ja 확보 → ② 측정 하네스 작성 → ③ #2 해결까지 완료했다.** 다만 ③에서 **Q4는 함께 닫히지 않았다** — #2("경량 신호만으로 트리아지가 성립하는가")는 Tier 0 질문이고, Q4("자가일관성 유사도를 무엇으로 재는가")는 LLM 재번역이 있어야 답할 수 있는 Tier 1 질문이라 이 벤치의 측정 범위 밖이다. 이전 판(위 #2 행의 "정정" 참고)은 이 둘을 같은 실험으로 묶어 적었는데, 그것이 틀렸다.
