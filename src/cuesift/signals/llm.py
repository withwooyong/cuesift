...
class SelfConsistency:
    ...
    **길이 편향이 있다 - 지금은 완화하지 않고 기록만 한다(Ruling P10).**
    ...
    en 8.1배, ja 6.4배 차이다. Recall@Budget은 순위로 정해지므로, 길이가
    점수를 지배하면 후보 큐가 "어려운 구간"이 아니라 "짧은 구간"으로
    채워질 위험이 있다. 길이 정규화 상수는 요구사항정의서 §11 R8(출처
    없는 수치 금지)에 걸리고, 판정은 벤치마크의 일인데 "벤치마크에
    Tier 1 태우기"는 이 작업 패키지의 명시적 비범위다 - 그래서 코드는
    바꾸지 않는다. 이 실측표는 §12 Q4(유사도 측정 수단 미결)의 근거에
    설계 §3.2의 7쌍 실측과 같은 방식으로 더해진다.
    """
