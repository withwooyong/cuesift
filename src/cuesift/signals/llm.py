    ...
    **길이 편향이 있다 - 지금은 완화하지 않고 기록만 한다(Ruling P10).**
    ...
    def _retranslate(self, seg: Segment, ctx: Tier1Context) -> list[str]:
        ...
        **비용은 세그먼트당 `ctx.samples`회다.** 후보 N개면 총 N ×
        ...
        호출에 묶을 수 없다. 실측(세그먼트 10개 × 원문 16자 · samples=3):
        이 구현은 30회 호출·10320자를 쓰는데, 세그먼트를 한 호출에
        묶었다면 3회·1599자였다 - 호출 10배, 프롬프트 6.45배(시스템
        프롬프트가 매 호출 반복되므로 실물 차이는 더 크다). **문자 수는
        원문 길이 L의 선형함수다** - 세그먼트 단위 `9840 + 30L`, 배치
        `1119 + 30L`(재리뷰가 L=3·L=15에서도 이 식대로 값이 나오는 것을
        확인했다). 위 수치는 L=16 조건에서만 유효하다 - **조건 없이
        인용하면 다른 예문에서 다른 값이 나와도 사실은 같은 식이 낸
        결과라는 것이 가려진다.** 이것은 설계 §4.1이
        ...
        **`seg`를 그대로 넘기지 않고 로컬 `index=0`으로 재번호한다
        ...
        문장·인덱스 2조 각 3회 = 6/6 재현): `index=1`("비가 그치기를
        기다렸다")·`index=5`("오늘 날씨가 정말 좋네요") 세그먼트는 항상
        `{"id": 0, ...}`를 답해 `parse_translations`가 "id가 누락됐다"로
        거부했고, 같은 세그먼트를 `index=0`으로만 바꾸면 3/3 성공했다.
        **온도 축과는 무관함이 확인됐다** - `_run_single`을
        `temperature=0.0`으로 돌려도 `index != 0`에서 똑같이 9/9
        실패했다(최종 브랜치 리뷰 D1). id 복사는 온도가 만드는 표본
        변이가 아니라 **형식 습관**이라는 뜻이다. 문장·index·모델·
        temperature를 이 독스트링에 남긴 이유는 재현 표가
        `.superpowers/sdd/`(git 추적 밖, 세션 종료 후 삭제됨)에만
        있으면 다음 사람이 재현할 수 없기 때문이다 - 추적되는 코드
        안에서 완결돼야 한다(최종 브랜치 리뷰 A10). 이것이 조용한
        이유는 `loader.py`가
