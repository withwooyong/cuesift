...
**이 모듈은 순수하다** — `ingest`·`typer`를 임포트하지 않는다. 판정의 기본 단위는
세그먼트 하나의 텍스트와 지속시간이다.

`Sequence`를 받는 함수는 둘인데 **이유가 서로 다르다.** `check_overlaps`는 트랙 전체를
봐야만 판정이 되고, `check_empty_cues`는 세그먼트 하나로 판정되지만 `check_track`이
겹침과 같은 방식으로 합칠 수 있도록 배치 API를 쓴다. 후자를 `check_text` 안에 넣지
않은 것은 `translate` 경로에서 FR-3.2의 `struct.empty`와 이중 계산되기 때문이다(설계 §4.2).
