...
def translate(
    ...
    no_cache: Annotated[
        ...
        typer.Option(
            ...
            # **"형식을 어긴 응답도 캐시된다"가 아니다.** 파킹 #13 이후
            # `invalid_response`는 `_discard_cached`가 지우고 캐시에 남는 것은
            # `empty_translation`이다. 옛 문구가 이 자리에 남아 docs/사용법.md로
            # 복사됐다 - 도움말은 사용자가 읽는 문서이므로 코드와 함께 고친다.
            help="캐시를 읽지도 쓰지도 않는다. 빈 번역은 캐시에 남으므로 "
    ...
    tier1: Annotated[
        ...
        typer.Option(
            ...
            # em dash(U+2014)를 쓰지 않는다(전역 제약, cp949 미인코딩).
            # 이 문구에 옵션 이름(`--`로 시작하는 토큰)을 넣지 않는다 - 색이
            # 켜진 CI에서 rich 하이라이터가 그것을 줄바꿈으로 쪼개 `--help`
            # 출력 테스트를 깨뜨린다(--review-format의 위 주석과 같은 전례).
            # 공백 없는 한글 덩어리를 10자 넘게 두지 않는다 - 전각이라 폭 88의
