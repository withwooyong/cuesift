...
def render_markdown(
    ...
    *,
    ...
    # 분류해 보니 **ja-ko는 41건 중 0건이 그 두 원인에 해당**했고(대신
    # 단위 환산·전각 숫자·한자 수사가 대부분), **"검출기 버그가 아니라"도
    # 거짓**이었다 — `struct.number_missing`의 숫자 추출(NFKC 정규화 없음)이
    # 전각 숫자(`５０`)와 반각 숫자(`50`)를 다른 값으로 봐서 실제로 검출기
    # 결함이 오탐의 일부를 차지한다(ja-ko의 31.7%). 이제는 알려진 요인을
    # 나열만 하고 **비중은 주장하지 않는다** — 매 실행마다 원인 분류를
    # 다시 하지 않으므로 비중이 실행마다 같다고 가정할 수 없다.
    lines += [
        "",
        "### hard fail 자연 오탐률",
        "",
        ...
        "단위를 환산하는 경우(`50파운드` → `20kg強`, `1분 30초` → `1分半`), 표기 체계가 다른 "
        "경우(한국어 만·억 vs 영어 billion), 자막 관행상 숫자를 단어로 푸는 경우(`30억개` → "
        "`three billion`), 그리고 **검출기의 알려진 한계**다 — `struct.number_missing`의 숫자 "
        "추출이 NFKC 정규화를 하지 않아 전각 숫자(`５０`)와 반각 숫자(`50`)를 다른 값으로 "
        '본다. 요구사항정의서 §9.1이 목표로 적은 "hard-fail 오탐 ≈ 0"의 실측값이며, 스펙 §6.4의 '
        "불변식 4는 이 값이 2%를 넘으면 결과를 내지 않고 실패한다.",
    ]
